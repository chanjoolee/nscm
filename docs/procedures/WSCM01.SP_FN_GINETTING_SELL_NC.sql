CREATE OR REPLACE PROCEDURE WSCM01.SP_FN_GINETTING_SELL_NC (IN_TYPE VARCHAR2 DEFAULT 'NCP') IS
/******************************************************************************
   NAME:       WSCM01.SP_FN_GINETTING_SELL_NC
   PURPOSE:    통합 물류 가용량네팅
   REVISIONS:
   VER     DATE        AUTHOR   DESCRIPTION
   ------- ----------  -------  ------------------------------------
   1.0     2014.01.15  feelsu.kim   신규 버전
 ...
 ******************************************************************************/
    ...
    C_PLANID                MST_GINETTING_SELL.PLANID%TYPE;
    C_SITEID                MST_GINETTING_SELL.SITEID%TYPE;
    C_SALESID               MST_GINETTING_SELL.SITEID%TYPE;
    C_ITEM                  MST_GINETTING_SELL.ITEM%TYPE;             
    C_WEEK                  MST_GINETTING_SELL.WEEK%TYPE;
    C_EXCEPTQ               MST_GINETTING_SELL.EXCEPTQ%TYPE;
    C_AVAIL                 MST_GINETTING_SELL.AVAIL%TYPE;                             
    C_DP                    MST_GINETTING_SELL.DP%TYPE;
    C_CUREXCEPT             MST_GINETTING_SELL.CUREXCEPT%TYPE;
    C_CURAVAIL              MST_GINETTING_SELL.CURAVAIL%TYPE;
...
--     V_WEEK4     := TO_CHAR(V_EFFSTARTDATE + 7*3, 'IYYYIW');    --2018.12.03 수정(V PLAN일 경우 당주 기준으로 변경)
    ...
    --! 백업 테이블 TURNCATE !--
    EXECUTE IMMEDIATE 'ALTER TABLE BUF_SOPROMISESRCNCP_POST TRUNCATE PARTITION PDPDEC_SELL';
    ...
    INSERT INTO BUF_SOPROMISESRCNCP_POST --SALESID(AP2ID)사용할수도 있으므로,,,일단 놔둠, 백업용 테이블
         ...
         SELECT    'DPDEC_SELL', ENTERPRISE, SOPROMISEID, A.PLANID, SALESORDERID, SOLINENUM, A.ITEM, A.QTYPROMISED, PROMISEDDELDATE
...
--          EXISTS (SELECT 'X' FROM V_MTA_SELLERMAP WHERE  ITEM = A.ITEM AND SITEID = A.SITEID)          
     ...
     DELETE FROM MST_GINETTING_SELL
     WHERE  PLANID = V_PLANID;
     
     
     --! GI NETTING 계산 과정 테이블에 가용량 + DEMAND INSERT !--
      D_SQL:='
      INSERT INTO MST_GINETTING_SELL
      (PLANID, SITEID, SALESID, ITEM, WEEK, RTF, GI, EXCEPTQ, AVAIL, DP, CUREXCEPT, CURAVAIL, INITDTTM, INITBY)
      WITH EXCEPT AS
      (
...
--         AND    NOT EXISTS (SELECT ''X'' FROM MVES_ITEMSITE_NC WHERE ITEM = A.ITEM AND SALESID = A.SALESID AND SITEID = A.SITEID)   --ESTORE 후보충 대상 제외 20210708
      ...
      , AVAIL AS(
        SELECT ITEM, SITEID, SALESID, WEEK, SUM(AVAILQTY) AVAILQTY, SUM(DPQTY) DPQTY
        FROM(
            -- 1. 가용량 (INVENTORY + INTRANSIT + DISTRIBUTIONORDERS)
            SELECT A.ITEM, A.SITEID, A.SALESID, A.WEEK, A.QTY AVAILQTY, 0 DPQTY
            ...
            -- 2. DEMAND
            UNION ALL
            SELECT  ITEM, SITEID, AP2ID SALESID, TO_CHAR(PROMISEDDELDATE, ''IYYYIW'') WEEK, 0, 
                    QTYPROMISED DPQTY
            FROM    EXP_SOPROMISESRCNCP A
            WHERE   1=1
       ...
       GROUP BY ITEM, SITEID, SALESID, WEEK
     )
     SELECT '''||V_PLANID||''' PLANID, A.SITEID, A.AP2ID, A.ITEM, B.WEEK, 
     ...
     FOR X IN(    
        SELECT RANK() OVER(PARTITION BY ITEM, SITEID, SALESID  ORDER BY WEEK ) RK,
               PLANID, ITEM, SITEID, SALESID, WEEK, EXCEPTQ, AVAIL, DP, CURAVAIL
        FROM   MST_GINETTING_SELL
        WHERE  PLANID = V_PLANID
        ORDER BY SITEID, SALESID, ITEM, WEEK
     )
     LOOP
        --! 첫 행이면 !--
        IF X.RK = 1 THEN
     ...
     --! DP와 AVAIL 비교하여 수량 FIX !--     
     FOR X IN(    
        SELECT RANK() OVER(PARTITION BY ITEM, SITEID, SALESID  ORDER BY WEEK ) RK,
               PLANID, ITEM, SITEID, SALESID, WEEK, EXCEPTQ, DP, CURAVAIL ,  CUREXCEPT
        FROM   MST_GINETTING_SELL
        WHERE  PLANID = V_PLANID
        ORDER BY SITEID, SALESID, ITEM, WEEK
     )
...
--                IF X.DP > X.CURAVAIL THEN
    ...
    FOR SITE IN (
        --W39 GIDP 651대
        SELECT SITEID, SALESID, ITEM, WEEK, GIDP 
        FROM   MST_GINETTING_SELL
        WHERE  PLANID = V_PLANID 
        AND   NVL(GIDP,0) >0
        ORDER BY SITEID, SALESID, ITEM, WEEK 
    ...
    LOOP
        ...
        FOR DP IN (
            ...
            SELECT RANK() OVER (ORDER BY RK.RNK, A.SALESID, A.DEMANDPRIORITY DESC, A.QTYPROMISED DESC, ROWNUM ) RNUM, 
                   ...
                   PREFERENCERANK, A.INITDTTM, A.INITBY, A.UPDTTM, A.UPBY, A.REASONCODE, A.OPTION_CODE, A.AP1ID
            ...
            AND    A.AP2ID = SITE.SALESID
            AND    A.SOLINENUM < 200
            --20220919 estore 제품 후처리 제외시, 계산에는 발췌해서 계산 후 분배시에 제외 하도록 조건 추가 req msjd95
            AND    NOT EXISTS (SELECT 'X' FROM MVES_ITEMSITE_NC WHERE ITEM = A.ITEM AND SALESID = A.SALESID AND SITEID = A.SITEID)
            )
...
--                DBMS_OUTPUT.PUT_LINE('3332 V_REMAINGIDP'||V_REMAINGIDP);     
   ...
   --! FOR LOOP 시작 !--
     FOR R IN (
         SELECT  SITEID, SALESID, ITEM, WEEK, 
...
--                       ,RANK() OVER (ORDER BY NVL(RK.RNK,0), A.SALESID, A.DEMANDPRIORITY DESC, A.QTYPROMISED DESC, ROWNUM ) RNUM
                         ...
                         ELSE
                            ...
                            BEGIN
                                 ...
                                 (  DP.ENTERPRISE, DP.SOPROMISEID, DP.PLANID, DP.SALESORDERID, /**++SOLINENUM +200 분리++**/DP.SOLINENUM+200, 
                                    ...
                                    DP.BUSINESSTYPE, DP.ROUTING_PRIORITY, DP.NO_SPLIT, DP.MAP_SATISFY_SS, 
                                    DP.PREALLOC_ATTRIBUTE, DP.BUILDAHEADTIME, DP.TIMEUOM, DP.AP2ID, DP.GC, 
                                    DP.MEASURERANK, DP.PREFERENCERANK, DP.INITDTTM, DP.INITBY, DP.UPDTTM, DP.UPBY || 'GINETTING_SELL', DP.REASONCODE, DP.OPTION_CODE, DP.AP1ID);
     ...
     BEGIN
         ...
         FROM(
             SELECT ITEM, SITEID, SALESID, WEEK, SUM(PROD) , SUM(DEV) 
             FROM(
                SELECT SITEID, AP2ID SALESID, ITEM, TO_CHAR(PROMISEDDELDATE,'IYYYIW') WEEK, 
                ...
                UNION ALL
                SELECT SITEID, SALESID, ITEM, WEEK, 0 PROD , GIDP DEV 
                FROM   MST_GINETTING_SELL A
                WHERE  PLANID = V_PLANID
                AND    NOT EXISTS (SELECT 'X' FROM MVES_ITEMSITE_NC WHERE ITEM = A.ITEM   AND SITEID = A.SITEID) --20210723 E-STORE 대상 제외
                )
                GROUP BY ITEM, SITEID, SALESID, WEEK
     ...
     BEGIN
         --! 3. GINETTING 후 빠지거나 중복된 수량이 없는지 점검 !--
         SELECT COUNT(*) INTO SMS_QTYTOTAL 
         FROM(
             SELECT ITEM, SITEID, SALESID, WEEK, SUM(PROD) , SUM(DEV) 
             FROM(
                SELECT SITEID, AP2ID SALESID, ITEM, TO_CHAR(PROMISEDDELDATE,'IYYYIW') WEEK, 
     ...
     IF SMS_QTYTOTAL > 0 THEN
        DBMS_OUTPUT.PUT_LINE('Error: check GINETTINGQTY');
     END IF;
