CREATE OR REPLACE PROCEDURE WSCM01.SP_FN_CHANNELSTOCK(IN_TYPE VARCHAR2 DEFAULT 'MAIN') IS
/******************************************************************************
...
******************************************************************************/
    ...
    V_PLANID            EXP_SOPROMISESRC.PLANID%TYPE;
...
--                                ,'300147',REPLACE(A.UDAITEM, 'INU','INS'), A.UDAITEM ) UDAITEM,
                ...
                FROM   EXP_DPGUIDE A, GUI_SALESHIERARCHY B, VUI_ITEMATTB C
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('V_PLANID : '||V_PLANID);


    BEGIN
    --! 이전 단계가 중복 수행되어 이미 OPTION_CODE가 1인 것이 없는지 체크 !--
    select count(*) into V_CHECK1 from EXP_SOPROMISESRC
    ...
    BEGIN
    --! 버퍼 테이블 비우기 !--
    EXECUTE IMMEDIATE 'TRUNCATE TABLE BUF_SOPROMISESRC_CH';

    EXECUTE IMMEDIATE 'Alter session ENABLE parallel DML';

    --! CHANNEL STOCK 하기전 데이터 백업!--
    INSERT /*+  APPEND  PARALLEL(T  5)  */  INTO BUF_SOPROMISESRC_CH T
            (ENTERPRISE, SOPROMISEID, PLANID, SALESORDERID, SOLINENUM, ITEM, QTYPROMISED, PROMISEDDELDATE
           ...
           , INITDTTM, INITBY, UPDTTM, UPBY, REASONCODE,OPTION_CODE, AP1ID)
    ...
    BEGIN
        ...
        exit WHEN dpguide_account%notfound;
            V_DPGUIDE := i.QTY;
            FOR DP IN (
                SELECT /*+ index( A X2_EXP_SOPROMISESRC) */ A.ROWID ROWA, ENTERPRISE, SOPROMISEID, PLANID, SALESORDERID, SOLINENUM, A.ITEM, QTYPROMISED, PROMISEDDELDATE
...
--                         DECODE(A.SITEID, 'S529', ITEM, 'S529WC27', ITEM, DEMANDPRIORITY),
                         DEMANDPRIORITY, ITEM, SALESORDERID DESC, SALESID DESC, QTYPROMISED DESC, TO_NUMBER(SOLINENUM), SITEID DESC
            )
            LOOP
                --! 가이드 수량 > 0 일때만 수행 !--
                IF V_DPGUIDE > 0 THEN
                    BEGIN
                        ...
                        ELSIF V_DPGUIDE < DP.QTYPROMISED THEN
                            ...
                            EXCEPTION WHEN DUP_VAL_ON_INDEX THEN
                                ...
                                SELECT TO_CHAR(MAX(TO_NUMBER(SOLINENUM)) + 1 )-- MAX(SOLINENUM) + 1
                                INTO V_SOLINENUM
                                FROM EXP_SOPROMISESRC
...
--                                DBMS_OUTPUT.PUT_LINE('2-1-1 CHANNEL STOCK 5 LEVEL SOLINENUM 중복발생 MAX+1 : '||DP.SALESORDERID||'***SOLINNUM'||V_SOLINENUM);
                    ...
                    EXCEPTION
                         ...
                         DBMS_OUTPUT.PUT_LINE('2-1-1. Backup Error : check '||SQLCODE||'_'||SQLERRM);
                         DBMS_OUTPUT.PUT_LINE('Error :  DP.SALESORDERID, '|| DP.SALESORDERID );
                         DBMS_OUTPUT.PUT_LINE('Error :  DP.SOLINENUM, '|| DP.SOLINENUM );
                         DBMS_OUTPUT.PUT_LINE('Error :  DP.ITEM, '|| DP.ITEM );
                         ROLLBACK;
                         WHEN OTHERS THEN
                         DBMS_OUTPUT.PUT_LINE('2-1-1 CHANNEL STOCK 5 LEVEL Error (DP_GUIDE > 0): check '||SQLCODE||'_'||SQLERRM);
                         DBMS_OUTPUT.PUT_LINE('2-1-1 CHANNEL STOCK 5 LEVEL DP.INFO  '||DP.SALESORDERID);
...
--                                DBMS_OUTPUT.PUT_LINE('2-2-1 CHANNEL STOCK 4 LEVEL SOLINENUM 중복발생 MAX+1 : '||DP.SALESORDERID||'***SOLINNUM'||V_SOLINENUM);
                    ...
                    EXCEPTION
                         ...
                         ROLLBACK;
                         WHEN DUP_VAL_ON_INDEX THEN
                         DBMS_OUTPUT.PUT_LINE('2-2-1. Backup Error : check '||SQLCODE||'_'||SQLERRM);
                         DBMS_OUTPUT.PUT_LINE('Error :  DP.SALESORDERID, '|| DP.SALESORDERID );
                         DBMS_OUTPUT.PUT_LINE('Error :  DP.SOLINENUM, '|| DP.SOLINENUM );
                         DBMS_OUTPUT.PUT_LINE('Error :  DP.ITEM, '|| DP.ITEM );
                         ROLLBACK;
...
--                                DBMS_OUTPUT.PUT_LINE('2-2-1 CHANNEL STOCK 3 LEVEL SOLINENUM 중복발생 MAX+1 : '||DP.SALESORDERID||'***SOLINNUM'||V_SOLINENUM);
                    ...
                    EXCEPTION
                         ...
                         ROLLBACK;
                         WHEN DUP_VAL_ON_INDEX THEN
                         DBMS_OUTPUT.PUT_LINE('2-3-1. Backup Error : check '||SQLCODE||'_'||SQLERRM);
                         DBMS_OUTPUT.PUT_LINE('Error :  DP.SALESORDERID, '|| DP.SALESORDERID );
                         DBMS_OUTPUT.PUT_LINE('Error :  DP.SOLINENUM, '|| DP.SOLINENUM );
                         DBMS_OUTPUT.PUT_LINE('Error :  DP.ITEM, '|| DP.ITEM );
                         ROLLBACK;
...
--                                                              END UDAITEM FROM EXP_DPGUIDE, COPYT WHERE MEASURE = 'ITEM_FCST' AND INT < 3
--                                             ) A, VUI_ITEMATTB B
...
--                                    WHERE A.UDAITEM = B.ITEM)
            ...
            SELECT * FROM (
              SELECT ROWA, PLANID, SALESORDERID, SOLINENUM, ENTERPRISE, SOPROMISEID, ITEM, QTYPROMISED, PROMISEDDELDATE, SITEID, SHIPTOID, SALESID, SALESLEVEL, DEMANDTYPE, WEEK,
                ...
                FROM ( SELECT A.ROWA,
                         ...
                         FROM DEMAND A, MST_INVENTORY_FNE B
            ...
            SELECT * FROM (
              SELECT ROWA, PLANID, SALESORDERID, SOLINENUM, ENTERPRISE, SOPROMISEID, ITEM, QTYPROMISED, PROMISEDDELDATE, SITEID, SHIPTOID, SALESID, SALESLEVEL, DEMANDTYPE, WEEK,
                ...
                FROM ( SELECT A.ROWA,
                        ...
                        FROM DEMAND_SELL A, MST_INVENTORY_FNE B
                        WHERE A.PLANID = B.PLANID(+)
                          AND A.SITEID = B.SITEID(+)
            ...
            ELSE
                ...
                IF V_AVAILQTY - (S.SUM_DEMAND - S.QTYPROMISED) > 0 AND S.QTYPROMISED > 0 AND  MOD(NVL(S.OPTION_CODE,0), 2) = 1 THEN
                    ...
                    BEGIN
                        ...
                        INSERT INTO EXP_SOPROMISESRC (PLANID, SALESORDERID, SOLINENUM, ENTERPRISE, SOPROMISEID, ITEM, QTYPROMISED, PROMISEDDELDATE, SITEID, SHIPTOID, SALESID, SALESLEVEL, DEMANDTYPE, WEEK,
                                                      CHANNELRANK, CUSTOMERRANK, PRODUCTRANK, DEMANDPRIORITY, TIEBREAK, GBM, GLOBALPRIORITY, LOCALPRIORITY, BUSINESSTYPE, ROUTING_PRIORITY, NO_SPLIT, MAP_SATISFY_SS,
        ...
        EXCEPTION WHEN DUP_VAL_ON_INDEX THEN
            ...
            DBMS_OUTPUT.PUT_LINE('3. Backup Error : check '||SQLCODE||'_'||SQLERRM);
            DBMS_OUTPUT.PUT_LINE('Error :  S.SALESORDERID, '|| S.SALESORDERID );
            DBMS_OUTPUT.PUT_LINE('Error :  S.SOLINENUM, '|| S.SOLINENUM );
            DBMS_OUTPUT.PUT_LINE('Error :  S.ITEM, '|| S.ITEM );
    ...
    BEGIN
        ...
        UPDATE /*+ index( A X2_EXP_SOPROMISESRC) */ EXP_SOPROMISESRC A
        SET    OPTION_CODE =  NVL(OPTION_CODE,0) +BIN_TO_NUM(1)
               ,UPBY = 'C/Stock_Upper'
        WHERE PLANID = V_PLANID
        AND (AP2ID,TO_CHAR(PROMISEDDELDATE, 'IYYYIW'),
             --20191106 INS/INU, XID/XSE 로직 삭제
--             DECODE(SITEID, 'L5N0',REPLACE(ITEM, 'INU','INS'), 'S529',REPLACE(ITEM, 'XSE','XID'), 'S529WC27',REPLACE(ITEM, 'XSE','XID'), ITEM)
               ITEM
             )
...
--                 DECODE(B.AP2ID , '300143', REPLACE(A.UDAITEM ,'XSE','XID'), '300147',REPLACE(A.UDAITEM, 'INU','INS'), A.UDAITEM)
                 A.UDAITEM
                   FROM EXP_DPGUIDE A, GUI_SALESHIERARCHY B
                  WHERE A.SALESID = B.SALESID
                    AND B.LEVELID = 5)
...
--  -- 2023.4.19 전사 SIG 산출시 Sales order 반영, MP 로직 삭제 필요  req by changmin.jun  C23040004761   23.5.18  로직 원복요청 C23050004533
    BEGIN
        ...
        WHERE ROWID IN (SELECT A.ROWID ROWIDS
                        FROM   EXP_SOPROMISESRC A, MTA_CODEMAP B
                        --        SET    OPTION_CODE =  NVL(OPTION_CODE,0)-BIN_TO_NUM(1)
                        --               ,UPBY = 'C/Stock_COM_ORD'
                        WHERE  MOD(NVL(OPTION_CODE,0), 2) = 1
                        AND    A.AP2ID = CODE1
            ...
            ELSIF V_AVAILQTY2 < SS.CHSTOCKQTY AND V_AVAILQTY2 > 0 THEN
               ...
               FOR QQ IN (SELECT ROW_NUMBER() OVER (ORDER BY DEMANDPRIORITY, QTYPROMISED) RNK, A.*
                          ...
                          BEGIN
                             ...
                             ELSIF V_AVAILQTY2 < QQ.QTYPROMISED AND V_AVAILQTY2 > 0 THEN
                                ...
                                BEGIN
                                    ...
                                    INSERT INTO EXP_SOPROMISESRC ( ENTERPRISE, SOPROMISEID, PLANID, SALESORDERID, SOLINENUM, ITEM, QTYPROMISED,
                                                                   PROMISEDDELDATE, SITEID, SHIPTOID, SALESID, SALESLEVEL, DEMANDTYPE, WEEK, CHANNELRANK, CUSTOMERRANK, PRODUCTRANK,
                                                                   DEMANDPRIORITY, TIEBREAK, GBM, GLOBALPRIORITY, LOCALPRIORITY, BUSINESSTYPE, ROUTING_PRIORITY,
                                                                   NO_SPLIT, MAP_SATISFY_SS, PREALLOC_ATTRIBUTE, BUILDAHEADTIME, TIMEUOM,
                          ...
                          EXCEPTION WHEN DUP_VAL_ON_INDEX THEN
                             ...
                             DBMS_OUTPUT.PUT_LINE('3. Backup Error : check '||SQLCODE||'_'||SQLERRM);
                             DBMS_OUTPUT.PUT_LINE('Error :  QQ.SALESORDERID, '|| QQ.SALESORDERID );
                             DBMS_OUTPUT.PUT_LINE('Error :  QQ.SOLINENUM, '|| QQ.SOLINENUM );
                             DBMS_OUTPUT.PUT_LINE('Error :  QQ.ITEM, '|| QQ.ITEM );
