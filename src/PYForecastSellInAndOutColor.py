...
str_instance = 'PYForecastSellInAndOutColor'
...
COL_SIN_FCST_COLOR_COND         = 'S/In FCST Color Condition'
...
COL_SOUT_FCST_COLOR_COND      = 'S/Out FCST Color Condition'
...
def fn_process_in_df_mst():
    ...
    if is_local: 
        ...
        file_to_df_mapping = {
            ...
            # 'df_in_Forecast_Rule.csv'                   :      STR_DF_RULE              ,
...
# ──────────────────────────────────────────────────────────────────────────────
# STEP-01 : RTS/EOS 전처리
#           · Version 과 RTS_ISVALID 컬럼 제거
#           · 사용 컬럼만 슬림화 + category 캐스팅
#           · 반환 DF 명 : df_fn_RTS_EOS  (STR_DF_FN_RTS_EOS)
# ──────────────────────────────────────────────────────────────────────────────
@_decoration_
def step01_preprocess_rts_eos(
        df_rts: pd.DataFrame,
        *,
        p_step_no: int | None = None,
        p_step_desc: str | None = None,
        p_df_name: str | None = None
) -> pd.DataFrame:
    """
    Parameters
    ----------
    df_rts : pd.DataFrame
        원본 `df_in_MST_RTS_EOS`.    Keyword-only
    ------------
    p_step_no / p_step_desc / p_df_name
        데코레이터용 메타정보(로그·CSV 출력).

    Returns
    -------
    pd.DataFrame
        RTS/EOS 핵심 컬럼만 남긴 DF   (STR_DF_FN_RTS_EOS)
    """
    # ── 0) 필요 컬럼 정의 ────────────────────────────────────────────────
    USE_COLS = [
    ...
    rts = (
        df_rts
        .drop(columns=[COL_VERSION, COL_RTS_ISVALID], errors='ignore')
        .loc[:, USE_COLS]                       # 컬럼 순서 고정
        .copy(deep=False)
    )

    # ── 2) 타입 캐스팅 (메모리 절감) ──────────────────────────────────────
    cat_cols = [COL_ITEM, COL_SHIP_TO, COL_RTS_STATUS, COL_EOS_STATUS]
    rts[cat_cols] = rts[cat_cols].astype('category')
...
def _series_to_pw(s: pd.Series) -> pd.Series:
    """중복 제거 + 캐시 변환 + map(99 %가량 가벼움)"""
    # ① 문자열로 통일 & NaT → NaN
    s_str = s.astype('string')
    uniq  = s_str.dropna().unique()
    lut   = {u: _pw_cached(u) for u in uniq}
    return s_str.map(lut, na_action='ignore')
...
def _bulk_add_week(base_series: pd.Series, offset: int) -> pd.Series:
    ...
    return base_str.map(lut, na_action='ignore')
...
def step02_convert_date_to_partial_week(
    ...
    for col in date_cols:                       # 벡터라이즈 변환
        # tgt[col] = v_sanitize_date_string(tgt[col])
        # tgt[col] = to_partial_week_datetime(tgt[col])
        # ①  시·분·초 제거 & 포맷 통일
        tgt[col] = v_sanitize_date_string(tgt[col].to_numpy())    
        # ②  Partial-Week 변환 (결측치 ⇒ NaN)
        # tgt[col] = v_to_partial_week(tgt[col].to_numpy())
        tgt[col] = _series_to_pw(tgt[col])

    # ── 2) RTS/EOS 기준 주차 산출 ──────────────────────────────────────
    # RTS :   COM → COM_DATE  /  그외 → DEV_DATE
    ...
    tgt[COL_RTS_WEEK] = tgt[COL_RTS_PARTIAL_WEEK].str.slice(stop=6)
    tgt[COL_EOS_WEEK] = tgt[COL_EOS_PARTIAL_WEEK].str.slice(stop=6)

    # 벡터화된 add-week 헬퍼
    def _safe_add_week(yyyyww: str, offset: int) -> str:
        """NaN / '' 방어 래퍼"""
        if not yyyyww:        # '', np.nan 등 False 로 평가
            return ''
        try:
            return common.gfn_add_week(yyyyww, offset)
        except Exception:     # 형식 불량인 경우도 '' 반환
            return ''
    v_add_week = np.vectorize(_safe_add_week, otypes=[object])

    # tgt[COL_RTS_WEEK_MINUST_1] = v_add_week(tgt[COL_RTS_WEEK], -1)
    # tgt[COL_RTS_WEEK_PLUS_3]   = v_add_week(tgt[COL_RTS_WEEK],  3)
    # tgt[COL_EOS_WEEK_MINUS_1]  = v_add_week(tgt[COL_EOS_WEEK], -1)
    # tgt[COL_EOS_WEEK_MINUS_4]  = v_add_week(tgt[COL_EOS_WEEK], -4)

    # ---------- 기존 numpy.vectorize 부분 대체 ----------
    ...
    tgt[COL_EOS_WEEK_MINUS_3] = _bulk_add_week(tgt[COL_EOS_WEEK], -3) # 추가 1125
    tgt[COL_EOS_WEEK_MINUS_4] = _bulk_add_week(tgt[COL_EOS_WEEK], -4)
    
    # '' → NaN 으로 되돌려 두면 이후 int32 캐스팅 시 0 처리 논리가 유지됩니다
    tgt.replace({'': np.nan}, inplace=True)

    # ── 4) RTS/EOS 초기 주차 & 기타 Helper ─────────────────────────────
    tgt[COL_RTS_INITIAL_WEEK] = tgt[COL_RTS_INIT_DATE].str.slice(stop=6)
    tgt[COL_EOS_INITIAL_WEEK] = tgt[COL_EOS_INIT_DATE].str.slice(stop=6)

    cur_ww = current_week[:6]                          # ‘202447A’ → ‘202447’
    tgt[COL_MAX_RTS_CURRENTWEEK] = np.where(
        tgt[COL_RTS_WEEK] < cur_ww, cur_ww, tgt[COL_RTS_WEEK]
    )
    df_time_pw = input_dataframes.get(STR_DF_TIME)
    max_week = df_time_pw[COL_TIME_PW].astype(str).max()
    
    tgt[COL_MIN_EOSINI_MAXWEEK] = np.where(
        tgt[COL_EOS_INITIAL_WEEK] < max_week[:6],
        tgt[COL_EOS_INITIAL_WEEK],
        max_week[:6]
    )

    tgt[COL_MIN_EOS_MAXWEEK] = np.where(
        tgt[COL_EOS_WEEK] < max_week[:6],
        tgt[COL_EOS_WEEK],
        max_week[:6]
    )
    ...
    obj_cols = tgt.select_dtypes(include='object').columns
...
def step03_1_prepare_asn_item(
    ...
    # 0) 기본 컬럼만 (좌측 원본 순서 유지)
    asn_use = (
        df_asn[[COL_SHIP_TO, COL_ITEM, COL_LOC]]
        .astype('category')
        .copy(deep=False)
    )    
    # 1) Ship-To → Lv6(COL_STD5) : LEFT MERGE (원본 순서 유지)
    asn_use = asn_use.merge(
        df_dim[[COL_SHIP_TO, COL_STD5]].astype({COL_SHIP_TO:'category', COL_STD5:'category'}),
        on=COL_SHIP_TO, how='left', sort=False, copy=False
    )

    # 2) Item Master : GBM/PG 매핑 (LEFT MERGE)
    asn_use = asn_use.merge(
        df_item_mst[[COL_ITEM, COL_GBM, COL_PG]].astype('category'),
        on=COL_ITEM, how='left', sort=False, copy=False
    )

    # 3-A) HA_EOP_FLAG (ITEMCLASS == 'X') : 3키 LEFT MERGE → 존재여부로 bool
    x_flag = (
        df_item_class.loc[df_item_class[COL_CLASS] == 'X',
                          [COL_ITEM, COL_SHIP_TO, COL_LOC]]
        .rename(columns={COL_SHIP_TO: COL_STD5})   # Lv6로 맞춰조인
        .drop_duplicates()
        .astype('category')
        .assign(__X__=True)
    )
    asn_use = asn_use.merge(
        x_flag, how='left', sort=False, copy=False,
        on=[COL_ITEM, COL_STD5, COL_LOC]
    )
    asn_use[COL_HA_EOP_FLAG] = asn_use['__X__'].fillna(False).astype('bool')
    asn_use.drop(columns='__X__', inplace=True)

    # 3-B) ITEMTAT : (Item, Loc) LEFT MERGE → NaN=0 → int32
...
# ─────────────────────────────────────────────────────────────────────────
@_decoration_
def step03_2_group_asn_to_ap2_item(
        df_asn_item: pd.DataFrame,   # ← step3-1 결과  df_fn_Sales_Product_ASN_Item
        df_dim:       pd.DataFrame    # ← step00 결과  df_fn_shipto_dim
) -> pd.DataFrame:
    """
    • Lv-6/7 Ship-To → Lv-3 우선, 없으면 Lv-2 로 부모 매핑
    • (부모 Ship-To, Item) 단위로
        - HA_EOP_FLAG, ESTOREACCOUNT : any(True)
        - TATTERM, TAT_SET           : max
    • dtype 최적화 (category / int32 / bool)
    """    
    # ── 0) 준비 : Ship-To ↔ Std1/Std2 LUT ndarray ──────────────────────
    dim_idx = df_dim.set_index(COL_SHIP_TO)
    # vector 로 뽑아두면 reindex 보다 훨씬 빠름
    std2_arr = dim_idx[COL_STD2].to_numpy()
    std1_arr = dim_idx[COL_STD1].to_numpy()

    pos_dim  = dim_idx.index.get_indexer(df_asn_item[COL_SHIP_TO])
    has_dim  = pos_dim >= 0

    parent_lv3 = np.where(has_dim, std2_arr[pos_dim], None)
    parent_lv2 = np.where(has_dim, std1_arr[pos_dim], None)

    # ── 1) RTS/EOS 가 존재하는 Lv-3 ↔ Lv-2 우선순위 look-up ─────────────
    # 변경사항 20250731: 무조건 3Lv 로 맞춘다.
    
    # rts_parent_set = set(df_fn_RTS_EOS[COL_SHIP_TO].unique())

    # Lv-3 먼저, 없으면 Lv-2
    # chosen_parent = np.where(
    #     np.isin(parent_lv3, list(rts_parent_set)), parent_lv3, parent_lv2
    # )
    
    # df_asn_item[COL_SHIP_TO] = pd.Categorical(chosen_parent)
    # del parent_lv2, parent_lv3, chosen_parent, std2_arr, std1_arr, pos_dim

    df_asn_item[COL_SHIP_TO] = pd.Categorical(parent_lv3) # 변경사항 20250731: 무조건 3Lv 로 맞춘다. 
    fn_log_dataframe(df_asn_item,'step03_2_temp_df_asn_item')
    del parent_lv2, parent_lv3, std2_arr, std1_arr, pos_dim
    gc.collect()
    ...
    # ── 3) dtype 캐스팅 & 반환 ──────────────────────────────────────────
    cat_cols = [COL_SHIP_TO, COL_ITEM]
    agg_df[cat_cols] = agg_df[cat_cols].astype('category')

    int_cols = [COL_TATTERM, COL_TAT_SET]
    agg_df[int_cols] = agg_df[int_cols].astype('int32', copy=False)
    ...
    for c in bool_cols:
        agg_df[c] = agg_df[c].astype('bool', copy=False)
...
def step04_build_rts_eos_week(
    ...
    * 기본 색상(19_GRAY) & Week-helper 컬럼 세팅
    ...
    df_out[COL_SIN_FCST_COLOR_COND] = (
        df_out[COL_SIN_FCST_COLOR_COND].astype('category')
    )
...
def step05_apply_color_rts_eos(
    ...
    def _week_to_int(s: pd.Series) -> np.ndarray:
        ...
        return (
            pd.to_numeric(s.astype(str), errors='coerce')   # NaN → NaN
              .fillna(0)
              .astype('int32')
              .to_numpy()
        )
...
def step06_apply_green_for_wireless_bas(
    ...
    week_int   = (
        pd.to_numeric(df_week[COL_CURRENT_ROW_WEEK].astype(str), errors='coerce')
...
def step07_apply_ha_eop_yellow(
    ...
    # ── 2) week → int32 배열
    def _w2i(s): 
        return pd.to_numeric(s.astype(str), errors='coerce').fillna(0).astype('int32').to_numpy()
...
def step08_1_apply_vd_leadtime(
    ...
    # ── 3) 비교·색상 덮어쓰기 ──────────────────────────────────────────
    wk_now_int = (
        pd.to_numeric(df_week[COL_CURRENT_ROW_WEEK].astype(str),
...
def step09_apply_gray_no_sellout(
    ...
    wk_now = pd.to_numeric(df_week[COL_CURRENT_ROW_WEEK].astype(str), errors='coerce').fillna(0).astype('int32').to_numpy()
    cw     = int(current_week[:6])
...
if __name__ == '__main__':
    ...
    try:
        ...
        if is_local:
            ...
            input_folder_name  = f'PYForecastSellInAndOutColor'
            output_folder_name = f'PYForecastSellInAndOutColor_0731'
            # # ME
        ...
        dict_log = {
            'p_step_no' : 1,
            'p_step_desc': 'Step 01 – RTS/EOS preprocessing'
        }
        df_fn_RTS_EOS = step01_preprocess_rts_eos(
            input_dataframes[STR_DF_RTS_EOS],
            **dict_log
        )
        fn_log_dataframe(df_fn_RTS_EOS, f'step01_{STR_DF_FN_RTS_EOS}')

        ################################################################################################################
        # Step 2 : Step1의 Result에 Time을 Partial Week 으로 변환
        ################################################################################################################
        ################################################################################################################
        # STEP 02 – 날짜 → Partial-Week 변환
        ################################################################################################################
        dict_log = {
            'p_step_no' : 2,
        ...
        df_fn_Sales_Product_ASN_Item = step03_1_prepare_asn_item(
            input_dataframes[STR_DF_ASN],               # 원본 ASN
            df_fn_shipto_dim,                           # Step00 결과
            input_dataframes[STR_DF_ITEMMST],           # Item Master
            input_dataframes[STR_DF_ITEMCLASS],         # Item Class
            input_dataframes[STR_DF_ITEMTAT],           # Item TAT
            # input_dataframes[STR_DF_ESTORE],            # eStore Ship-To
            **dict_log
        )
        ...
        dict_log = {
            'p_step_no' : 3.2,
            'p_step_desc': 'Step 03-2 – group ASN to AP2×Item'
        }
        df_fn_Sales_Product_ASN_Group = step03_2_group_asn_to_ap2_item(
            df_fn_Sales_Product_ASN_Item,       # ← step3-1 결과
            df_fn_shipto_dim,     # ← step00 결과
            **dict_log
        )
