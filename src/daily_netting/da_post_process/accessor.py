...
from daily_netting.da_post_process.constant.general_constant import SalesOrderLP as SOLP
...
from daily_netting.da_post_process.constant.dim_constant import NettingLPPlanBatch as NLPPB
...
from daily_netting.da_post_process.utils.common import make_sequential_id
...
class Accessor:
    ...
    '''
    def __init__(
            self, df_dim_item, df_dim_location, df_dim_sales_domain,
        ...
        self.df_dim_sales_domain = df_dim_sales_domain[SD.LIST_COLUMN]
        ...
        # general
        self.df_account_info = df_account_info[AI.LIST_COLUMN]
        self.df_df_alloc_map = df_df_alloc_map[DFAM.LIST_COLUMN]
        self.df_customer_rank = df_customer_rank[CR.LIST_COLUMN]
        self.df_product_rank = df_product_rank[ProdR.LIST_COLUMN]
        self.df_bod = df_bod[BOD.LIST_COLUMN]
        self.df_sales_order_lp = df_sales_order_lp[SOLP.LIST_COLUMN]

        # da
        self.df_da_preference_rank = df_da_preference_rank[DAPR.LIST_COLUMN]
        self.df_da_item_seller_map = df_da_item_seller_map[DAISM.LIST_COLUMN]
        self.df_da_seller_map = df_da_seller_map[DASM.LIST_COLUMN]
        self.df_region_asn = df_region_asn[RASN.LIST_COLUMN]
        self.df_local_measure = df_local_measure[LM.LIST_COLUMN]
        self.df_local_sales_order = df_local_sales_order[LSaO.LIST_COLUMN]
        self.df_gi = df_gi[GI.LIST_COLUMN]
        self.df_shipment = df_shipment[S.LIST_COLUMN]
    ...
    def _convert_empty_type(self):
        ...
        if self.df_bod.empty:
            self.df_bod = self.df_bod.astype({
        ...
        if self.df_da_preference_rank.empty:
            self.df_da_preference_rank = self.df_da_preference_rank.astype({
                DAPR.PREFERENCERANK: 'float64',
            })
        if self.df_local_measure.empty:
            self.df_local_measure = self.df_local_measure.astype({
                LM.MEASUREQTY: 'float64',
            })
        if self.df_local_sales_order.empty:
            self.df_local_sales_order = self.df_local_sales_order.astype({
                LSaO.QTY: 'float64',
            })
        if self.df_gi.empty:
            self.df_gi = self.df_gi.astype({
                GI.GIQTY: 'float64',
            })
        if self.df_shipment.empty:
            self.df_shipment = self.df_shipment.astype({
