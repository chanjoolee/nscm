...
str_instance = 'PYForecastSellInAndOutColorDelta'
...
def step01_preprocess_rts_eos_delta(
    ...
    # 1) 사용할 컬럼만 슬림화 (+존재하는 Delta 컬럼만 선별)
    ...
    # 3) Output(3) 사본 구성 (Version 포함)
    #    - 스펙상 Version은 단일(CWV_DP)로 가정 → 카테고리로 통일
    df_output_RTS_EOS = df_norm.copy(deep=False)
    ...
    df_fn_RTS_EOS = df_norm.drop(columns=[c for c in drop_cols if c in df_norm.columns], errors='ignore')
...
if __name__ == '__main__':
    ...
    try:
        ...
        if is_local:
            ...
            output_folder_name = f'PYForecastSellInAndOutColorDelta'
            # # ME
        ...
        df_fn_RTS_EOS, df_output_RTS_EOS = step01_preprocess_rts_eos_delta(
            input_dataframes[STR_DF_RTS_EOS_DELTA],
            Version
        )
