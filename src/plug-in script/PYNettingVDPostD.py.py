...
if nscm.G_IS_Local:
    ...
    log_file_name = f'C:\\workspace\\log\\PostD_VD'
    if os.path.exists(log_file_name):
        os.remove(log_file_name)
    file_handler = nscm.logging.FileHandler(log_file_name, encoding='UTF-8')
    nscm.logging.getLogger('o9_logger').addHandler(file_handler)
...
try:
    ...
    logger.PrintDF(df_out_ap1_delivery_plan ,'df_out_ap1_delivery_plan '.strip(),p_row_num=20)	
    logger.PrintDF(df_out_ap1_short_reason  ,'df_out_ap1_short_reason  '.strip(),p_row_num=20)
    logger.PrintDF(df_out_demand            ,'df_out_demand            '.strip(),p_row_num=20)
...
if nscm.G_IS_Local:
    ...
    import shutil
    output_folder = f'C:\\workspace\\Output\\PYNettingMXPostD_{additional_path}'
    os.makedirs(output_folder, exist_ok=True)
    df_out_ap1_delivery_plan.to_csv(f'{output_folder}\\df_out_ap1_delivery_plan.csv', index=False)
    df_out_ap1_short_reason.to_csv(f'{output_folder}\\df_out_ap1_short_reason.csv', index=False)
    df_out_demand.to_csv(f'{output_folder}\\df_out_demand.csv', index=False)

    shutil.copyfile(log_file_name, os.path.join(output_folder, os.path.basename(log_file_name)))
