...
* 변경이력
    - 2025.10.22 / lee kyung young / create  / PYForecastAR.py copy 후 로직 변경 : 1) AP2 -> AP1 전환   2) P4W 계산 삭제   3) Flooring FCST Delta 처리 추가
...
* Input
    - df_in_h = pd.read_csv('./input/dp_fcst_arkam/df_in_h.csv')
    ...
    - df_in_d = pd.read_csv('./input/dp_fcst_arkam/df_in_d.csv')
        Select (
            [DP Data Seq].[DP Data Seq]
    ...
    - df_in_sellin = pd.read_csv('./input/dp_fcst_arkam/df_in_sellin.csv')
        Select (
    ...
    - df_in_sellout = pd.read_csv('./input/dp_fcst_arkam/df_in_sellout.csv')
        Select (
            [Sales Domain].[Ship To]
            * [Item].[Item]
            * [Location].[Location]
    ...
    - df_in_transit_time = pd.read_csv('./input/dp_fcst_arkam/df_in_transit_time.csv')
        Select (
            [Sales Domain].[Ship To]
            * [Item].[Product Group]
            * [Location].[Location]
    ...
    - df_in_split_ratio = pd.read_csv('./input/dp_fcst_arkam/df_in_split_ratio.csv')
        Select (
            [Sales Domain].[Ship To]
            * [Item].[Item]
            * [Location].[Location]
    ...
    - df_in_estimated_price_USD = pd.read_csv('./input/dp_fcst_arkam/df_in_estimated_price_USD.csv')
        Select (
            [Sales Domain].[Ship To]
            * [Item].[Item]
            * [Time].[Partial Week]
    ...
    - * 25.11.15 해당 Input 삭제
      df_in_exchange_rate = pd.read_csv('./input/dp_fcst_arkam/df_in_exchange_rate.csv')
        Select (
    ...
    + df_in_chinv = pd.read_csv('./input/dp_fcst_arkam/df_in_chinv.csv')
        Select (
            [Sales Domain].[Ship To]
            * [Item].[Item]
        ) on row,
        ({
            Measure.[(Simul)Ch Inv PW_AP1],
            Measure.[(Simul)Ch Inv_Inc Floor PW_AP1]
        }) on column
        where {
            [Version].[Version Name],
    ...
    - df_in_item = pd.read_csv('./input/dp_fcst_arkam/df_in_item.csv')
        Select (
             [Item].[Item GBM]
            * [Item].[Item Std1]
            * [Item].[Item]
    ...
    - df_in_sales = pd.read_csv('./input/dp_fcst_arkam/df_in_sales.csv')
        Select (
    ...
    + df_in_target_WOS = pd.read_csv('./input/dp_fcst_arkam/df_in_target_WOS.csv')
        Select (
            [Sales Domain].[Ship To]
            * [Item].[Item]
            * [Time].[Week]
    ...
    + df_in_floor_fcst = pd.read_csv('./input/dp_fcst_arkam/df_in_floor_fcst.csv')
        Select (
            [Sales Domain].[Ship To]
            * [Item].[Item]
            * [Time].[Week]
    ...
    - df_in_version = pd.read_csv('./input/dp_fcst_arkam/df_in_version.csv')
        select ([Version].[Version Name]
        );


    - df_in_gi_bl_intransit_fcst = pd.read_csv('./input/dp_fcst_arkam/df_in_gi_bl_intransit_fcst.csv')
        Select (
            [Sales Domain].[Ship To]
            * [Item].[Item]
            * [Location].[Location]
    ...
    - df_in_fcst_gi_assortment = pd.read_csv('./input/dp_fcst_arkam/df_in_fcst_gi_assortment.csv')
        Select ([Version].[Version Name]
            * [Item].[Item]
            * [Sales Domain].[Ship To]
            * [Location].[Location]
...
* Output
    - df_output
        df_output_Sellin_gi
                ...
                ({
                    Measure.[S/In FCST(GI)_AP1],
                    Measure.[S/In FCST(GI) AMT_USD_AP1],
                    Measure.[S/In FCST(GI) AMT_Local_AP1],
                    Measure.[(Simul)S/In FCST_AP1]
                }) on column;
        ...
        df_output_Sellin_bl
                ...
                ({
                    Measure.[S/In FCST(BL)_AP1],
                    Measure.[S/In FCST(BL) AMT_USD_AP1],
                    Measure.[S/In FCST(BL) AMT_Local_AP1],
                    Measure.[S/In FCST(BL) AMT_Region_AP1]
                }) on column;
        ...
        df_output_Sellout
                ...
                ({
                    Measure.[S/Out FCST_AP1],
                    Measure.[S/Out FCST AMT_USD_AP1],
                    Measure.[S/Out FCST AMT_Local_AP1],
                    Measure.[S/Out FCST Modify_AP1]
                }) on column;
        ...
        df_output_chinv
                ...
                ({
                    Measure.[(Simul)Ch Inv PW_AP1],
                    Measure.[(Simul)Ch Inv_Inc Floor PW_AP1],
                    Measure.[(Simul)Target Ch Inv PW_AP1]
                }) on column;
        ...
        df_output_wos
                ...
                ({
                    Measure.[(Simul)WOS_AP1],
                    Measure.[(Simul)WOS_Inc Floor_AP1]
                }) on column;
...
* Flow Summary
    ...
    Step 01. S/In FCST(GI) , S/Out FCST Delta값 DataFrame 형태로 변환 및 전체데이터와 합치기, Delta 별도 관리
        ...
        3	Version.[Version Name]^Sales Domain.[Ship To]^Item.[Item]^Location.[Location]^Time.[Partial Week]	S/In FCST(GI)_AP1
        4	Version.[Version Name]^Sales Domain.[Ship To]^Item.[Item]^Location.[Location]^Time.[Partial Week]	S/Out FCST_AP1
        5   Version.[Version Name]^Sales Domain.[Ship To]^Item.[Item]^Time.[Week]	Flooring FCST
        ...
        5   1   408158^RR39C7AF5WW/EF^202521A:2000|202522A:2000|202522B:2000

        - df_in_sellin S/In FCST(GI) 와 합치기
        - df_in_sellout 이용해서 S/Out FCST 와 합치기
        - Flooring FCST Data 합치기

    Step 02. S/Out FCST Data 가공
        - S/Out FCST_AP1 Aggregation (Ship To, Item, Week)
        - P4W로직 제외함 -> S/Out Actual(P4W) 값 생성 : df_in_sellout 에서 S/Out FCST 값 가져와서 P4W 생성
        - (Week Avg)S/Out FCST F4W_AP1 값 생성 : S/Out FCST 값 가져와서 F4W로 만들어야함. 이때 Location은 필요 없음
    ...
    Step 04. Ch Inv 값 연산
	    - S/In FCST(BL)_AP1 Aggregation (Ship To, Item, Week)
	    - (Simul)Ch Inv PW_AP1, (Simul)Ch Inv_Inc loor_AP1 : Measure.[(Simul)Ch Inv PW_AP1] = if ( SUM(Measure.[(Simul)Ch Inv PW_AP1]@(Time.#.leadoffset(-1)), Measure.[(Copy)S/In FCST(BL)_AP1], Measure.[(Copy)S/Out FCST_AP1] * -1) < 0 ) then 0 else SUM(Measure.[(Simul)Ch Inv PW_AP1]@(Time.#.leadoffset(-1)), Measure.[(Copy)S/In FCST(BL)_AP1], Measure.[(Copy)S/Out FCST_AP1] * -1);"
	    - P4W로직 제외함 -> (Simul)P4W Ch Inv PW, (Simul)P4W Ch Inv_Inc Floor PW : Measure.[(Simul)P4W Ch Inv PW] = if ( SUM(Measure.[(Simul)P4W Ch Inv PW]@(Time.#.leadoffset(-1)), Measure.[(Copy)S/In FCST(BL)_AP1], Measure.[(Copy)S/Out Actual(P4W)] * -1) < 0) then 0else SUM(Measure.[(Simul)P4W Ch Inv PW]@(Time.#.leadoffset(-1)), Measure.[(Copy)S/In FCST(BL)_AP1], Measure.[(Copy)S/Out Actual(P4W)] * -1);
	    - (25.09.25)  (Simul)Ch Inv PW 을 PW으로 변환

    Step 05. WOS 값 연산
	    - (Input 3) WOS 계산을 위한 정보 <- WOS는 Recurrence가 아니기 때문에 데이터 가져올 필요 X, 매번 새로운 Data 넣어주어야함. Q : Delta만 저장할 것인지. 아니면 52주 매번 전체 저장할 것인지
        - (Simul)WOS_AP1, (Simul)WOS_Inc Floor_AP1 : Measure.[(Simul)WOS_AP1] = Safedivide(Measure.[(Simul)Ch Inv PW_AP1], Measure.[(Week Avg)S/Out FCST F4W_AP1], 0);
        - P4W로직 제외함 -> (Simul)P4W WOS, (Simul)P4W WOS_Inc Floor : Measure.[(Simul)P4W WOS] = Safedivide(Measure.[(Simul)P4W Ch Inv PW], Measure.[(Week Avg)S/Out Actual(P4W) F4W], 0);
        - WOS 는 Week -> Partial Week 구성 (Copy)

    Step 06. (25.09.25)  (Simul)S/In FCST_AP1 연산
        - Target Ch WOS 값 전처리
        - Target Ch Inv 값 구하기
        - Target Ch Inv 값 PW 구하기
        - (Simul) S/In FCST_AP1 값 연산
        - Partial Week 연산
        Measure.[(Simul)Target Ch Inv PW_AP1] = sum(Measure.[(Copy)S/Out FCST_AP1]@([Time].[Week].#.LeadOffset(1)), measure.[(Copy)S/Out FCST_AP1]@([Time].[Week].#.LeadOffset(2)), measure.[(Copy)S/Out FCST_AP1]@([Time].[Week].#.LeadOffset(3)), measure.[(Copy)S/Out FCST_AP1]@([Time].[Week].#.LeadOffset(4))) / 4 * coalesce(measure.[(Copy)Target Ch WOS], 0);
    ...
    Step 08. Output 생성
        ...
        - (Simul)Ch Inv PW_AP1, (Simul)Ch Inv_Inc loor_AP1
        - (Simul)WOS_AP1, (Simul)WOS_Inc Floor_AP1
        - Flooring FCST

* Execution

    EXEC plugin instance [PYForecastARKAM]
    for measures {
    Measure.[S/In FCST(GI)_AP1], Measure.[S/In FCST(GI) AMT_USD_AP1], Measure.[S/In FCST(GI) AMT_Local_AP1], Measure.[(Simul)S/In FCST_AP1],
    Measure.[S/In FCST(BL)_AP1], Measure.[S/In FCST(BL) AMT_USD_AP1], Measure.[S/In FCST(BL) AMT_Local_AP1], Measure.[S/In FCST(BL) AMT_Region_AP1],
    Measure.[S/Out FCST_AP1], Measure.[S/Out FCST AMT_USD_AP1], Measure.[S/Out FCST AMT_Local_AP1],
    Measure.[(Simul)Ch Inv PW_AP1], Measure.[(Simul)Ch Inv_Inc Floor PW_AP1], Measure.[(Simul)Target Ch Inv PW_AP1],
    Measure.[(Simul)WOS_AP1], Measure.[(Simul)WOS_Inc Floor_AP1]
    }
    using scope (
    ...
    * [Item].[Item]
    * [Location].[Location]
    * [Time].[Week].filter (#.Key >= &CurrentWeek.element(0).leadoffset(-5).Key && #.Key < &CurrentWeek.element(0).leadoffset(57).Key) )
...
# logger 설정
########################################################################################################################
logger = NSCMCommon.G_Logger(p_py_name='PYForecastARKAM.py')
...
try:
    ...
    dict_in_split_ratio = {'Sales Domain.[Ship To]': 'string'
        ...
        , 'S/In FCST(GI) Split Ratio_AP1': 'float64'}

    dict_in_chinv = {'Sales Domain.[Ship To]': 'string'
        , 'Item.[Item]': 'string'
        , '(Simul)Ch Inv PW_AP1': 'float64'
        , '(Simul)Ch Inv_Inc Floor PW_AP1': 'float64'}
    ...
    dict_in_fcst_gi_assortment = {'Version.[Version Name]': 'string'
        ...
        , 'Location.[Location]': 'string'
        , 'S/In FCST(GI) Assortment_AP1': 'float64'}
    ...
    ###########################################
    # df_tmp_sellin_fcst_gi : S/In FCST(GI)_AP1
    ###########################################
    where_sellin_fcst_gi = df_tmp_detail['Upload Header_Measure'] == 'S/In FCST(GI)_AP1'
    ...
    list_sellin = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Location.[Location]', 'Time.[Partial Week]', 'S/In FCST(GI)_AP1']
    if 'S/In FCST(GI)_AP1' in dict_in_h:
        list_sellin = dict_in_h['S/In FCST(GI)_AP1']

    df_in_sellin_fcst_gi = pd.DataFrame(arr_all_col, columns=list_sellin)
    df_in_sellin_fcst_gi['S/In FCST(GI)_AP1'] = df_in_sellin_fcst_gi['S/In FCST(GI)_AP1'].astype('int64')

    logger.Note(f'Step 1-2)')
    fn_PrintDF(p_df=df_in_sellin_fcst_gi, p_df_name=f'df_in_sellin_fcst_gi ({df_in_sellin_fcst_gi.shape[0]}) - df_in_h와 df_in_d 를 이용해 UI에서 입력된 Delta 값 DataFrame 변환 ["S/In FCST(GI)_AP1"] 검증 완료')
    ...
    df_org_sellin = df_org_sellin.groupby(by=list_org_groupby)['S/In FCST(GI)_AP1'].sum().reset_index()

    logger.Note(f'Step 1-3)')
    fn_PrintDF(p_df=df_org_sellin, p_df_name=f'df_org_sellin ({df_org_sellin.shape[0]})')

    ###########################################
    # df_tmp_sellout_fcst   : S/Out FCST_AP1
    ###########################################
    where_sellout_fcst = df_tmp_detail['Upload Header_Measure'] == 'S/Out FCST_AP1'
    ...
    list_sellout = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Location.[Location]', 'Time.[Partial Week]', 'S/Out FCST_AP1']
    if 'S/Out FCST_AP1' in dict_in_h:
        list_sellout = dict_in_h['S/Out FCST_AP1']

    df_in_sellout_fcst = pd.DataFrame(arr_all_col, columns=list_sellout)
    df_in_sellout_fcst['S/Out FCST_AP1'] = df_in_sellout_fcst['S/Out FCST_AP1'].astype('int64')

    # add 20251016
    df_in_sellout_fcst['S/Out FCST Modify_AP1'] = 1
    df_in_sellout_fcst['S/Out FCST Modify_AP1'] = df_in_sellout_fcst['S/Out FCST Modify_AP1'].astype('int64')

    logger.Note(f'Step 1-4)')
    fn_PrintDF(p_df=df_in_sellout_fcst, p_df_name=f'df_in_sellout_fcst ({df_in_sellout_fcst.shape[0]}) - df_in_h와 df_in_d 를 이용해 UI에서 입력된 Delta 값 DataFrame 변환 ["S/Out FCST_AP1"] 검증 완료')

    ###########################################
    # df_tmp_floor_fcst   : Flooring FCST
    ###########################################
    where_floor_fcst = df_tmp_detail['Upload Header_Measure'] == 'Flooring FCST'
    df_tmp_floor_fcst = df_tmp_detail.loc[where_floor_fcst].copy(deep=True)
    df_tmp_floor_fcst = df_tmp_floor_fcst.reset_index(drop=True)

    irow = 0
    icol = 0
    arr_all_col = None
    for i, row in df_tmp_floor_fcst.iterrows():
        str_row = row['Upload Data_Data']
        if 'Version.[Version Name]' in row['Upload Header_Dimension']:
            str_row = f"{CWV}^" + str_row
        arr_row = str_row.split('^')
    ...
    logger.Note(f"Floor arr_all_col.reshape : {arr_all_col}")

    list_floor = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Week]', 'Flooring FCST']
    if 'Flooring FCST' in dict_in_h:
        list_floor = dict_in_h['Flooring FCST']

    df_in_floor_fcst_param = pd.DataFrame(arr_all_col, columns=list_floor)
    df_in_floor_fcst_param['Flooring FCST'] = df_in_floor_fcst_param['Flooring FCST'].astype('int64')

    logger.Note(f'Step 1-5)')
    fn_PrintDF(p_df=df_in_floor_fcst, p_df_name=f'df_in_floor_fcst ({df_in_floor_fcst.shape[0]}) - df_in_h와 df_in_d 를 이용해 UI에서 입력된 Delta 값 DataFrame 변환 ["Flooring FCST"] 검증 완료')

    ###########################################
    # 1. df_in_sellin : S/In FCST(GI) 와 합치기
    ###########################################
    list_sellin_key = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Location.[Location]', 'Time.[Partial Week]']
    ...
    df_in_sellin['S/In FCST(GI)_AP1_y'] = df_in_sellin['S/In FCST(GI)_AP1_y'].fillna(value=0)
    df_in_sellin['S/In FCST(GI)_AP1_y'] = df_in_sellin['S/In FCST(GI)_AP1_y'].astype('int64')

    df_in_sellin['S/In FCST(GI)_AP1'] = np.where(df_in_sellin['S/In FCST(GI)_AP1_y'].notna()
                                                 , df_in_sellin['S/In FCST(GI)_AP1_y']
                                                 , df_in_sellin['S/In FCST(GI)_AP1'])

    df_in_sellin = df_in_sellin.drop('S/In FCST(GI)_AP1_y', axis=1)

    logger.Note(f'Step 1-5)')
    fn_PrintDF(p_df=df_in_sellin, p_df_name=f'df_in_sellin ({df_in_sellin.shape[0]})')

    ###########################################
    # 2. df_in_sellout : S/Out FCST 와 합치기
    ###########################################
    ...
    df_in_sellout['S/Out FCST_AP1'] = df_in_sellout['S/Out FCST_AP1'].fillna(value=0)
    df_in_sellout['S/Out FCST_AP1'] = df_in_sellout['S/Out FCST_AP1'].astype('int64')

    df_in_sellout['S/Out FCST_AP1'] = np.where(df_in_sellout['S/Out FCST_AP1_y'].notna()
                                               , df_in_sellout['S/Out FCST_AP1_y']
                                               , df_in_sellout['S/Out FCST_AP1'])

    ###########################################
    # 3. df_in_floor : floor FCST 와 합치기
    ###########################################
    list_floor_key = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Week]']
    df_in_floor_fcst_param[list_floor_key] = df_in_floor_fcst_param[list_floor_key].astype('string')

    df_in_floor_fcst = df_in_floor_fcst.merge(df_in_floor_fcst_param, how='left', on=list_floor_key, suffixes=('', '_y'))

    df_in_floor_fcst['Flooring FCST'] = df_in_floor_fcst['Flooring FCST'].fillna(value=0)
    df_in_floor_fcst['Flooring FCST'] = df_in_floor_fcst['Flooring FCST'].astype('int64')

    df_in_floor_fcst['Flooring FCST'] = np.where(df_in_floor_fcst['Flooring FCST_y'].notna()
                                                 , df_in_floor_fcst['Flooring FCST_y']
                                                 , df_in_floor_fcst['Flooring FCST'])
    ...
    fn_PrintDF(p_df=df_in_floor_fcst, p_df_name=f'df_in_floor_fcst({df_in_floor_fcst.shape[0]}) - df_in_floor_fcst 이용해서 Flooring FCST 와 합치기 검증 완료')
    ...
    df_in_sellout_agg = df_in_sellout_agg.groupby(by=list_sellout_groupby)['S/Out FCST_AP1'].sum().reset_index()
    ...
    df_tmp_sellout_agg_p4w_mx = df_tmp_sellout_agg_p4w_mx.groupby(by=list_mx_groupby)['S/Out FCST_AP1'].mean().reset_index()
    ...
    df_tmp_sellout_agg_p4w_ce = df_tmp_sellout_agg_p4w_ce.groupby(by=list_ce_groupby)['S/Out FCST_AP1'].mean().reset_index()
    ...
    # p4w 결과
    ##################################################
    df_out_sellout_agg_p4w = pd.DataFrame(columns=['Sales Domain.[Ship To]', 'Item.[Item]', 'S/Out FCST_AP1'])
    ...
    df_out_sellout_agg_p4w.rename(columns={'S/Out FCST_AP1': 'S/Out Actual(P4W)'}, inplace=True)
    ...
    ########################################################################################
    # - (Week Avg)S/Out FCST F4W_AP1 값 생성 : df_in_sellout 에서 S/Out FCST 값 가져와서 F4W로 만들어야함. 이때 Location은 필요 없음
    ...
    df_in_sellout_agg_f4w['f4w'] = df_in_sellout_agg_f4w['S/Out FCST_AP1'].rolling(window=4).mean()
    ...
    df_out_sellout_agg_f4w['S/Out FCST_AP1'] = df_out_sellout_agg_f4w['f4w']

    list_sellout_f4w = list_sellout_groupby + ['S/Out FCST_AP1']
    df_out_sellout_agg_f4w = df_out_sellout_agg_f4w[list_sellout_f4w]

    # (Week Avg)S/Out FCST F4W_AP1
    df_out_sellout_agg_f4w.rename(columns={'S/Out FCST_AP1': '(Week Avg)S/Out FCST F4W_AP1'}, inplace=True)

    # End Step log
    logger.Step(2, 'S/Out FCST data 가공')
    fn_PrintDF(p_df=df_in_sellout_agg, p_df_name=f'df_in_sellout_agg ({df_in_sellout_agg.shape[0]})')
    # fn_PrintDF(p_df=df_out_sellout_agg_p4w, p_df_name=f'df_out_sellout_agg_p4w ({df_out_sellout_agg_p4w.shape[0]}) - S/Out FCST_AP1 Aggregation (Ship To, Item, Week) 검증 완료')
    fn_PrintDF(p_df=df_out_sellout_agg_f4w, p_df_name=f'df_out_sellout_agg_f4w ({df_out_sellout_agg_f4w.shape[0]}) - S/Out FCST_AP1 Aggregation (Ship To, Item, Week) 검증 완료')
    ...
    list_sellin_sum = ['S/In FCST(GI)_AP1', 'S/In FCST(BL)_AP1']
    ...
    # 'S/In FCST(GI)_AP1' -> 'S/In FCST(BL)_AP1'
    df_tmp_sellin_bl['S/In FCST(BL)_AP1'] = df_tmp_sellin_bl['S/In FCST(GI)_AP1']
    ...
    for i, iweek in enumerate(list_transit_w):
        ...
        df_tmp_sellin_bl.loc[where_transit_w & where_transit_w2, 'S/In FCST(BL)_AP1'] = np.nan

    # shift
    # list_transit_w = df_in_transit_time['DP Master GI BL LT Transit Time W'].unique().tolist()
    for i, iweek in enumerate(list_transit_w):
        where_transit_w = (df_tmp_sellin_bl['DP Master GI BL LT Transit Time W'] == iweek)
        df_tmp_sellin_bl.loc[where_transit_w, 'S/In FCST(BL)_AP1'] = df_tmp_sellin_bl.loc[where_transit_w].groupby(by=list_bl)['S/In FCST(BL)_AP1'].shift(periods=iweek)
    ...
    df_tmp_sellin_bl['S/In FCST(BL)_AP1'] = np.where(df_tmp_sellin_bl['S/In FCST(BL)_AP1'].notna()
                                                     , df_tmp_sellin_bl['S/In FCST(BL)_AP1']
                                                     , df_tmp_sellin_bl['S/In FCST(GI)_AP1'])

    list_tmp_bl = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Location.[Location]', 'Time.[Week]'
        , 'S/In FCST(BL)_AP1', 'DP Master GI BL LT Transit Time W', 'org_value']
    ...
    # 비율 default 0
    df_out_sellin_bl['S/In FCST(GI) Split Ratio_AP1'] = df_out_sellin_bl['S/In FCST(GI) Split Ratio_AP1'].fillna(value=0)

    df_out_sellin_bl['ratio_sum'] = df_out_sellin_bl.groupby(by=list_bl)['S/In FCST(GI) Split Ratio_AP1'].transform('sum')

    # ratio 가 0 or nan 이면 1:1로 분배 될수 있게 1로 설정한다.
    df_out_sellin_bl['S/In FCST(GI) Split Ratio_AP1'] = np.where((df_out_sellin_bl['S/In FCST(GI) Split Ratio_AP1'] == 0) &
                                                                 (df_out_sellin_bl['cum_max'] == 1)
                                                                 , 1
                                                                 , df_out_sellin_bl['S/In FCST(GI) Split Ratio_AP1'])

    # -> 2. S/In FCST(GI) Split Ratio_AP1 0이거나 Null인 경우 -> A/B 주차에 값이 하나라도 있으면 0, 둘다 0 or Null 이면 -> 1
    df_out_sellin_bl['S/In FCST(GI) Split Ratio_AP1'] = np.where((df_out_sellin_bl['S/In FCST(GI) Split Ratio_AP1'] == 0) &
    ...
    df_out_sellin_bl['sum_ratio'] = df_out_sellin_bl.groupby(by=list_bl_group)['S/In FCST(GI) Split Ratio_AP1'].transform('sum')

    # partial week 1W
    where_first = (df_out_sellin_bl['cum_max'] == 2) & (df_out_sellin_bl['cum_count'] == 0)
    df_out_sellin_bl.loc[where_first, 'S/In FCST(BL)_AP1_calc'] = df_out_sellin_bl.loc[where_first, 'S/In FCST(BL)_AP1'] * (
            df_out_sellin_bl.loc[where_first, 'S/In FCST(GI) Split Ratio_AP1'] / df_out_sellin_bl.loc[where_first, 'sum_ratio'])

    df_out_sellin_bl.loc[where_first, 'S/In FCST(BL)_AP1_calc'] = np.round(df_out_sellin_bl.loc[where_first, 'S/In FCST(BL)_AP1_calc'])

    df_out_sellin_bl.loc[where_first, 'S/In FCST(BL)_AP1'] = df_out_sellin_bl.loc[where_first, 'S/In FCST(BL)_AP1_calc']

    logger.Note(f'Step 3-7)')
    fn_PrintDF(p_df=df_out_sellin_bl, p_df_name=f'df_out_sellin_bl ({df_out_sellin_bl.shape[0]})')

    df_out_sellin_bl['S/In FCST(BL)_AP1_calc'] = df_out_sellin_bl['S/In FCST(BL)_AP1_calc'].shift(periods=1)
    ...
    df_out_sellin_bl.loc[where_second, 'S/In FCST(BL)_AP1_calc'] = df_out_sellin_bl.loc[where_second, 'S/In FCST(BL)_AP1'] - df_out_sellin_bl.loc[where_second, 'S/In FCST(BL)_AP1_calc']
    df_out_sellin_bl.loc[where_second, 'S/In FCST(BL)_AP1'] = df_out_sellin_bl.loc[where_second, 'S/In FCST(BL)_AP1_calc']
    ...
    df_out_sellin_bl['GI BL Intransit'] = df_out_sellin_bl['GI BL Intransit'].fillna(0).astype('int64')

    df_out_sellin_bl["S/In FCST(BL)_AP1"] = np.where( (df_out_sellin_bl['DP Master GI BL LT Transit Time W'] == 0)
                                                      & (df_out_sellin_bl['Time.[Partial Week]'].str.startswith(CurrentWeek))
                                                      , df_out_sellin_bl["S/In FCST(BL)_AP1"] + df_out_sellin_bl["GI BL Intransit"]
                                                      , df_out_sellin_bl["S/In FCST(BL)_AP1"] )

    # 3 - 컬럼 정리
    list_all_bl = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Location.[Location]', 'Time.[Partial Week]', 'S/In FCST(BL)_AP1', 'org_value']
    ...
    df_tmp_sellin_bl_agg = df_tmp_sellin_bl.groupby(by=list_bl_agg)['S/In FCST(BL)_AP1'].sum().reset_index()
    ...
    if not df_tmp_sellin_bl_agg.empty:
        ...
        df_tmp_sellin_bl_agg['S/In FCST(BL)_AP1'] = np.nan
        df_tmp_sellin_bl_agg['S/In FCST(BL)_AP1'] = df_tmp_sellin_bl_agg['S/In FCST(BL)_AP1'].astype('float64')
    ...
    ##################################################################################
    # - (Simul)Ch Inv PW_AP1, (Simul)Ch Inv_Inc floor_AP1 : Measure.[(Simul)Ch Inv PW_AP1] =
    # (25.09.25) (Simul)Ch Inv PW_AP1 : Measure.[(Simul)Ch Inv PW_AP1] = Measure.[(Simul)Ch Inv PW_AP1]@(Time.#.leadoffset(-1)) + Measure.[(Copy)S/In FCST(BL)_AP1] - Measure.[(Copy)S/Out FCST_AP1]
    # (25.09.25) (Simul)Ch Inv_Inc Floor PW_AP1 : Measure.[(Simul)Ch Inv_Inc Floor PW_AP1] = Measure.[(Simul)Ch Inv_Inc Floor PW_AP1]@(Time.#.leadoffset(-1)) + Measure.[(Copy)S/In FCST(BL)_AP1] - Measure.[(Copy)S/Out FCST_AP1] + Measure.[Flooring FCST]
    ##################################################################################
    list_simul_ch = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Week]', 'S/In FCST(BL)_AP1', 'S/Out FCST_AP1',
                     '(Simul)Ch Inv PW_AP1', '(Simul)Ch Inv_Inc Floor PW_AP1']
    ...
    if len(list_simul_week) > 1:
        ...
        df_simul_sellin_bl_agg.loc[where_simul_nan, '(Simul)Ch Inv PW_AP1'] = np.nan
        df_simul_sellin_bl_agg.loc[where_simul_nan, '(Simul)Ch Inv_Inc Floor PW_AP1'] = np.nan
    ...
    df_simul_sellin_bl_agg = df_simul_sellin_bl_agg.merge(df_in_floor_fcst, how='left', on=list_simul_bl_agg_key)
    ...
    if not df_simul_sellin_bl_agg.empty:
        arr_simul = [df_simul_sellin_bl_agg['(Simul)Ch Inv PW_AP1'].values[0]]
        for i in range(1, len(df_simul_sellin_bl_agg.index)):
            value = df_simul_sellin_bl_agg['(Simul)Ch Inv PW_AP1'].values[i]
            week = df_simul_sellin_bl_agg['Time.[Week]'].values[i]

            if (week >= CurrentWeek) & (week <= CurrentWeek_52):
                sin = df_simul_sellin_bl_agg['S/In FCST(BL)_AP1'].values[i]
                sout = df_simul_sellin_bl_agg['S/Out FCST_AP1'].values[i]
                ...
                arr_simul.append(value)
        df_simul_sellin_bl_agg['(Simul)Ch Inv PW_AP1'] = arr_simul
        # df_simul_sellin_bl_agg['(Simul)Ch Inv_Inc Floor PW_AP1'] = arr_simul_floor

    ##################################################################################
    # - 25.11.15 Measure.[(Simul)Ch Inv_Inc Floor PW_AP1] = Measure.[(Simul)Ch Inv PW_AP1] + Measure.[Flooring FCST]
    ##################################################################################
    df_simul_sellin_bl_agg['(Simul)Ch Inv_Inc Floor PW_AP1'] = np.where(df_simul_sellin_bl_agg['(Simul)Ch Inv PW_AP1'].isna()
                                                                        , np.nan
                                                                        , df_simul_sellin_bl_agg['(Simul)Ch Inv PW_AP1'] + df_simul_sellin_bl_agg['Flooring FCST'])

    logger.Note(f'Step 4-3)')
    fn_PrintDF(p_df=df_simul_sellin_bl_agg, p_df_name=f'df_simul_sellin_bl_agg ({df_simul_sellin_bl_agg.shape[0]}) - (Simul)Ch Inv PW_AP1, (Simul)Ch Inv_Inc floor_AP1 검증 완료')

    '''
    ##################################################################################
    # - (Simul)P4W Ch Inv PW, (Simul)P4W Ch Inv_Inc Floor PW : Measure.[(Simul)P4W Ch Inv PW] =
    # (25.09.25) (Simul)P4W Ch Inv PW : Measure.[(Simul)P4W Ch Inv PW] = Measure.[(Simul)P4W Ch Inv PW]@(Time.#.leadoffset(-1)) + Measure.[(Copy)S/In FCST(BL)_AP1] - Measure.[(Copy)S/Out Actual(P4W)]
    # (25.09.25) (Simul)P4W Ch Inv_Inc Floor PW : Measure.[(Simul)P4W Ch Inv_Inc Floor PW] = Measure.[(Simul)P4W Ch Inv_Inc Floor PW]@(Time.#.leadoffset(-1)) + Measure.[(Copy)S/In FCST(BL)_AP1] - Measure.[(Copy)S/Out Actual(P4W)] + Measure.[Flooring FCST]
    ##################################################################################
    list_simul_p4w = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Week]', 'S/In FCST(BL)_AP1',
    ...
    list_simul_p4w = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Week]', 'S/In FCST(BL)_AP1', 'S/Out Actual(P4W)',
                      '(Simul)P4W Ch Inv PW', '(Simul)P4W Ch Inv_Inc Floor PW']
    ...
    if not df_simul_sellin_p4w_agg.empty:
        ...
        for i in range(1, len(df_simul_sellin_p4w_agg.index)):
            ...
            if (week >= CurrentWeek) & (week <= CurrentWeek_52):
                ...
                value = arr_simul[i - 1] + df_simul_sellin_p4w_agg['S/In FCST(BL)_AP1'].values[i] - df_simul_sellin_p4w_agg['S/Out Actual(P4W)'].values[i]
                arr_simul.append(value)
    ...
    df_partial_week = df_simul_sellin_bl_agg[['Time.[Week]']].copy(deep=True)
    df_partial_week = df_partial_week.drop_duplicates()
    ...
    df_simul_sellin_pw_agg = df_simul_sellin_bl_agg.merge(df_partial_week, how='inner', on=['Time.[Week]'])
    ...
    # 	- (25.09.25)  (Simul)WOS_AP1, (Simul)WOS_Inc Floor_AP1 : Measure.[(Simul)WOS_AP1] = if ( Measure.[(Simul)Ch Inv PW_AP1] < 0 ) then 0 else Safedivide(Measure.[(Simul)Ch Inv PW_AP1], Measure.[(Week Avg)S/Out FCST F4W_AP1], 0);
    ####################################################################################################################
    ...
    # '(Simul)WOS_AP1' = '(Simul)Ch Inv PW_AP1' / '(Week Avg)S/Out FCST F4W_AP1'
    df_wos_sellin_bl_agg['(Simul)WOS_AP1'] = np.where(df_wos_sellin_bl_agg['(Simul)Ch Inv PW_AP1'] < 0
                                                      , 0
                                                      , np.where(df_wos_sellin_bl_agg['(Week Avg)S/Out FCST F4W_AP1'].isna() | (df_wos_sellin_bl_agg['(Week Avg)S/Out FCST F4W_AP1'] == 0)
                                                                 , 0
                                                                 , df_wos_sellin_bl_agg['(Simul)Ch Inv PW_AP1'] / df_wos_sellin_bl_agg['(Week Avg)S/Out FCST F4W_AP1'])
                                                      )

    # '(Simul)WOS_Inc Floor_AP1' = '(Simul)Ch Inv_Inc Floor PW_AP1' / '(Week Avg)S/Out FCST F4W_AP1'
    df_wos_sellin_bl_agg['(Simul)WOS_Inc Floor_AP1'] = np.where(df_wos_sellin_bl_agg['(Simul)Ch Inv_Inc Floor PW_AP1'] < 0
                                                                , 0
                                                                , np.where(df_wos_sellin_bl_agg['(Week Avg)S/Out FCST F4W_AP1'].isna() | (df_wos_sellin_bl_agg['(Week Avg)S/Out FCST F4W_AP1'] == 0)
                                                                           , 0
                                                                           , df_wos_sellin_bl_agg['(Simul)Ch Inv_Inc Floor PW_AP1'] / df_wos_sellin_bl_agg['(Week Avg)S/Out FCST F4W_AP1'])
                                                                )

    logger.Note(f'Step 5-1)')
    fn_PrintDF(p_df=df_wos_sellin_bl_agg, p_df_name=f'df_wos_sellin_bl_agg ({df_wos_sellin_bl_agg.shape[0]}) - (Simul)WOS_AP1, (Simul)WOS_Inc Floor_AP1 검증 완료')
    ...
    list_out_wos = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Partial Week]', '(Simul)WOS_AP1', '(Simul)WOS_Inc Floor_AP1']
    ...
    # - (25.09.25)  (Simul)WOS_AP1, (Simul)WOS_Inc Floor_AP1 :
    #   Measure.[(Simul)WOS_AP1] = if ( Measure.[(Simul)Ch Inv PW_AP1] < 0 ) then 0
    #                              else Safedivide(Measure.[(Simul)Ch Inv PW_AP1], Measure.[(Week Avg)S/Out FCST F4W_AP1], 0);
    ##############################################
    ...
    ####################################################################################################################
    # Step 6) (Simul)S/In FCST_AP1 연산
    # 	- Target Ch WOS 값 전처리
    ####################################################################################################################
    # * S/Out Delta가 있는 경우에 로직 수행                               -> df_in_sellout_fcst
    # * Step2의 S/Out FCST_AP1 Aggregation (Ship To, Item, Week) 된 값 -> df_in_sellout_agg
    #   과 (Week Avg)S/Out FCST F4W_AP1의 값을 가져온다.                 -> df_out_sellout_agg_f4w
    ...
    # * (Simul)Target Ch Inv PW_AP1 = Target Ch WOS * (Week Avg)S/Out FCST F4W_AP1
    # * Target Ch WOS 가 Null 이면 0 처리
    # * 실제로 W+0 ~ W + 52 까지 값 연산
    df_out_wos_target['(Simul)Target Ch Inv PW_AP1'] = np.where(df_out_wos_target['Target Ch WOS'].isna()
                                                                , 0
                                                                , df_out_wos_target['Target Ch WOS'] * df_out_wos_target['(Week Avg)S/Out FCST F4W_AP1']
                                                                )
    ...
    list_out_wos_target_pw = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Partial Week]',
                              '(Week Avg)S/Out FCST F4W_AP1', 'S/Out FCST_AP1',
                              'Target Ch WOS', '(Simul)Target Ch Inv PW_AP1']
    ...
    # - (Simul) S/In FCST_AP1 값 연산
    ############################################################
    # * Step 4에서 계산된 (Simul)Ch Inv PW_AP1 값 추가 (W-1 값까지)
    # * (Simul)S/In FCST_AP1 = (Simul)Target Ch Inv PW_AP1 + S/Out FCST_AP1 - (Simul)Ch Inv PW_AP1(-1)
    # * 실제로 W+0 ~ W + 52 까지 값 연산
    # * (Simul)Target Ch Inv PW_AP1 는 Step7 Output 구성시 필요함
    ...
    list_tmp_sellin_agg = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Week]', '(Simul)Ch Inv PW_AP1']
    ...
    df_out_simul_sellin_agg['(Simul)S/In FCST_AP1'] = np.nan

    ##################################################################
    # (Simul)S/In FCST_AP1 계산 :
    ##################################################################
    if not df_out_simul_sellin_agg.empty:
        arr_simul = [df_out_simul_sellin_agg['(Simul)S/In FCST_AP1'].values[0]]
        for i in range(1, len(df_out_simul_sellin_agg.index)):
            value = df_out_simul_sellin_agg['(Simul)S/In FCST_AP1'].values[i]
            week = df_out_simul_sellin_agg['Time.[Week]'].values[i]

            if (week >= CurrentWeek) & (week <= CurrentWeek_52):
                chinv = df_out_simul_sellin_agg['(Simul)Target Ch Inv PW_AP1'].values[i]
                sout  = df_out_simul_sellin_agg['S/Out FCST_AP1'].values[i]
                chinv_pre = df_out_simul_sellin_agg['(Simul)Ch Inv PW_AP1'].values[i - 1]
        ...
        df_out_simul_sellin_agg['(Simul)S/In FCST_AP1'] = arr_simul

    logger.Note(f'Step 6-4)')
    fn_PrintDF(p_df=df_out_simul_sellin_agg, p_df_name=f'df_out_simul_sellin_agg ({df_out_simul_sellin_agg.shape[0]}) - (Simul) S/In FCST_AP1 값 연산')

    ############################################################
    # - (Simul) S/In FCST_AP1  Location 및 Partial Week 분배 연산
    ############################################################
    list_sellin_fcst = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Week]', '(Simul)S/In FCST_AP1']
    # where_sellin_fcst = df_out_simul_sellin_agg['(Simul)S/In FCST_AP1'].notna()
    ...
    if not df_tmp_split_ratio.empty:
        ...
        df_out_simul_sellin_pw['S/In FCST(GI) Split Ratio_AP1'] = 1
    ...
    list_out_simul_pw_all = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Location.[Location]', 'Time.[Partial Week]',
                             '(Simul)S/In FCST_AP1', 'S/In FCST(GI) Split Ratio_AP1']
    ...
    df_out_simul_sellin_pw['ratio_sum'] = df_out_simul_sellin_pw.groupby(by=list_out_simul_pw_cum)['S/In FCST(GI) Split Ratio_AP1'].transform('sum')

    # Initial Float Value, Cumulative error, Final Value 추가
    df_out_simul_sellin_pw['Initial Float Value'] = df_out_simul_sellin_pw['(Simul)S/In FCST_AP1'] * df_out_simul_sellin_pw['S/In FCST(GI) Split Ratio_AP1'] / df_out_simul_sellin_pw['ratio_sum']
    ...
    df_out_simul_sellin_step6['(Simul)S/In FCST_AP1'] = df_out_simul_sellin_step6['Final Value']

    # End Step log
    logger.Step(6, '(Simul)S/In FCST_AP1 연산')
    fn_PrintDF(p_df=df_out_simul_sellin_pw, p_df_name=f'df_out_simul_sellin_pw ({df_out_simul_sellin_pw.shape[0]}) - (Simul) S/In FCST_AP1  Location 및 Partial Week 분배 연산')
    fn_PrintDF(p_df=df_out_simul_sellin_step6, p_df_name=f'df_out_simul_sellin_step6 ({df_out_simul_sellin_step6.shape[0]})')

    ####################################################################################################################
    # Step 7) AMT 값 연산
    # 	- S/In FCST(GI) Delta 값 AMT 계산 : USD, Local
    #   - (25.12.06) : Delta 인 S/In FCST에 대해서 S/In FCST(GI) Modify_AP1 Measure 추가 및 값 1 적용
    ####################################################################################################################
    # price_USD
    ...
    # calc
    df_out_amt_sellin_fcst_gi['S/In FCST(GI) AMT_USD_AP1'] = df_out_amt_sellin_fcst_gi['S/In FCST(GI)_AP1'] * df_out_amt_sellin_fcst_gi['Estimated Price_USD']
    df_out_amt_sellin_fcst_gi['S/In FCST(GI) AMT_Local_AP1'] = df_out_amt_sellin_fcst_gi['S/In FCST(GI)_AP1'] * df_out_amt_sellin_fcst_gi['Estimated Price_Local']
    df_out_amt_sellin_fcst_gi['S/In FCST(GI) Modify_AP1'] = 1
    ...
    # calc
    df_out_amt_sellin_bl['S/In FCST(BL) AMT_USD_AP1'] = df_out_amt_sellin_bl['S/In FCST(BL)_AP1'] * df_out_amt_sellin_bl['Estimated Price_USD']
    df_out_amt_sellin_bl['S/In FCST(BL) AMT_Local_AP1'] = df_out_amt_sellin_bl['S/In FCST(BL)_AP1'] * df_out_amt_sellin_bl['Estimated Price_Local']
    # 주석처리
    # df_out_amt_sellin_bl['S/In FCST(BL) AMT_Region_AP1'] = df_out_amt_sellin_bl['Exchange Rate_Region'] * df_out_amt_sellin_bl['S/In FCST(BL) AMT_USD_AP1']
    ...
    # calc
    df_out_amt_sellout_fcst['S/Out FCST AMT_USD_AP1'] = df_out_amt_sellout_fcst['S/Out FCST_AP1'] * df_out_amt_sellout_fcst['Estimated Price_USD']
    df_out_amt_sellout_fcst['S/Out FCST AMT_Local_AP1'] = df_out_amt_sellout_fcst['S/Out FCST_AP1'] * df_out_amt_sellout_fcst['Estimated Price_Local']

    #######################################################
    # (20251027) add
    # - (Week Avg)S/Out FCST F4W PW_AP1 생성 (Grain : Item, ShipTo, Location, Partial Week, Version)
    ...
    list_out_sellout_agg_f4w_pw = ['Sales Domain.[Ship To]', 'Item.[Item]', 'Location.[Location]', 'Time.[Partial Week]',
                                   '(Week Avg)S/Out FCST F4W PW_AP1']
    if df_out_sellout_agg_f4w.empty:
        ...
        df_out_sellout_agg_f4w_pw.rename(columns={'(Week Avg)S/Out FCST F4W_AP1': '(Week Avg)S/Out FCST F4W PW_AP1'}, inplace=True)
    ...
    logger.Note(f'Step 7-3) (Week Avg)S/Out FCST F4W PW_AP1 생성')
    ...
    dict_output_gi = {'Version.[Version Name]': 'string'
        ...
        , 'Location.[Location]': 'string'
        , 'Time.[Partial Week]': 'string'
        , 'S/In FCST(GI)_AP1': 'float64'
        , 'S/In FCST(GI) AMT_USD_AP1': 'float64'
        , 'S/In FCST(GI) AMT_Local_AP1': 'float64'
        , 'S/In FCST(GI) Modify_AP1': 'int'}
    ...
    dict_output_bl = {'Version.[Version Name]': 'string'
        ...
        , 'Location.[Location]': 'string'
        , 'Time.[Partial Week]': 'string'
        , 'S/In FCST(BL)_AP1': 'float64'
        , 'S/In FCST(BL) AMT_USD_AP1': 'float64'
        , 'S/In FCST(BL) AMT_Local_AP1': 'float64'}
        # , 'S/In FCST(BL) AMT_Region_AP1': 'float64'}
    ...
    dict_output_sellout = {'Version.[Version Name]': 'string'
        ...
        , 'Location.[Location]': 'string'
        , 'Time.[Partial Week]': 'string'
        , 'S/Out FCST_AP1': 'float64'
        , 'S/Out FCST AMT_USD_AP1': 'float64'
        , 'S/Out FCST AMT_Local_AP1': 'float64'
        , 'S/Out FCST Modify_AP1': 'int64'}
    ...
    dict_output_chinv = {'Version.[Version Name]': 'string'
        ...
        , '(Simul)Ch Inv PW_AP1': 'float64'
        , '(Simul)Ch Inv_Inc Floor PW_AP1': 'float64'
        , '(Simul)Target Ch Inv PW_AP1': 'float64'}

    dict_output_wos = {'Version.[Version Name]': 'string'
        , 'Sales Domain.[Ship To]': 'string'
        , 'Item.[Item]': 'string'
        , 'Time.[Partial Week]': 'string'
        , '(Simul)WOS_AP1': 'float64'
        , '(Simul)WOS_Inc Floor_AP1': 'float64'}

    dict_output_floor = {'Version.[Version Name]': 'string'
        , 'Sales Domain.[Ship To]': 'string'
        , 'Item.[Item]': 'string'
        , 'Time.[Week]': 'string'
        , 'Flooring FCST': 'float64'}
    ...
    dict_output_sellout_f4w = {'Version.[Version Name]': 'string'
        ...
        , 'Location.[Location]': 'string'
        , 'Time.[Partial Week]': 'string'
        , '(Week Avg)S/Out FCST F4W PW_AP1': 'float64'}


    ########################################################
    # (Output 1) (25.09.25) (Simul)S/In FCST_AP1 추가
    list_gi = ['Version.[Version Name]', 'Sales Domain.[Ship To]', 'Item.[Item]', 'Location.[Location]', 'Time.[Partial Week]',
               'S/In FCST(GI)_AP1', 'S/In FCST(GI) AMT_USD_AP1', 'S/In FCST(GI) AMT_Local_AP1', 'S/In FCST(GI) Modify_AP1']
    ...
    list_bl = ['Version.[Version Name]', 'Sales Domain.[Ship To]', 'Item.[Item]', 'Location.[Location]', 'Time.[Partial Week]',
               'S/In FCST(BL)_AP1', 'S/In FCST(BL) AMT_USD_AP1', 'S/In FCST(BL) AMT_Local_AP1']
               # 'S/In FCST(BL)_AP1', 'S/In FCST(BL) AMT_USD_AP1', 'S/In FCST(BL) AMT_Local_AP1', 'S/In FCST(BL) AMT_Region_AP1']
    ...
    list_sellout = ['Version.[Version Name]', 'Sales Domain.[Ship To]', 'Item.[Item]', 'Location.[Location]', 'Time.[Partial Week]',
                    'S/Out FCST_AP1', 'S/Out FCST AMT_USD_AP1', 'S/Out FCST AMT_Local_AP1', 'S/Out FCST Modify_AP1']
    ...
    #                                                , on=['Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Week]'])
    df_output_chinv = df_simul_sellin_bl_agg.copy(deep=True)

    list_ch = ['Version.[Version Name]', 'Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Partial Week]',
               '(Simul)Ch Inv PW_AP1', '(Simul)Ch Inv_Inc Floor PW_AP1', '(Simul)Target Ch Inv PW_AP1']
    df_output_chinv['Version.[Version Name]'] = CWV

    # (Simul)Target Ch Inv PW_AP1 추가 및 pw 변경
    ...
    #                                                , on=['Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Partial Week]'])
    df_output_wos = df_out_wos_sellin_bl_agg.copy(deep=True)
    ...
    ########################################################
    # (Output 6)
    df_output_floor = df_in_floor_fcst_param.copy(deep=True)

    list_floor = ['Version.[Version Name]', 'Sales Domain.[Ship To]', 'Item.[Item]', 'Time.[Week]',
                  'Flooring FCST']
    df_output_floor['Version.[Version Name]'] = CWV

    df_output_floor = df_output_floor[list_floor]
    df_output_floor = df_output_floor.astype(dtype=dict_output_floor)
    ...
    fn_PrintDF(p_df=df_output_floor, p_df_name=f'df_output_floor ({df_output_floor.shape[0]})')
