...
# 20251230 추가
COL_SALES_PRODUCT_ASN_DELTA_LEAF  = 'Sales Product ASN Delta Leaf Flag'
...
<<<<<<< HEAD
    ...
    # ── 0) 입력 방어 : 공백이면 정상종료 유도 (빈 DF 반환) ───────────────────
    if df_asn_delta is None or df_asn_delta.empty:
        logger.info("[Step 1-1] Input 6 (df_in_Sales_Product_ASN_Delta) is empty. Skip all steps and finish normally.")
        df_empty = pd.DataFrame(columns=out_cols)
        for c in out_cols:
...
<<<<<<< HEAD
    ...
    df_out = df[[COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_SALES_PRODUCT_ASN, COL_SALES_PRODUCT_ASN_LEAF]]
...
<<<<<<< HEAD
    ...
    df[COL_VERSION] = version
    df_out = df[[COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_SALES_PRODUCT_ASN_DELTA,COL_SALES_PRODUCT_ASN_DELTA_LEAF]]
...
<<<<<<< HEAD
    ...
    if est_set:
        mask_es = df_asn[COL_SHIP_TO].astype("string").isin(est_set).to_numpy()
    else:
        mask_es = np.zeros(len(df_asn), dtype=bool)
    # ────────────────────────────────────────────────────────────────────────
    # 2-1) Core: vectorised 룰 매칭 → 4개 Tag buffer 생성 (S/In용)
    # ────────────────────────────────────────────────────────────────────────
    ship_np = df_asn[COL_SHIP_TO].astype("string").to_numpy()
...
<<<<<<< HEAD
        ...
        if mask.dtype != bool:
            # 0/1, float, int 등 모두 대응
            mask = pd.to_numeric(mask, errors="coerce").fillna(0).astype(np.int8).astype(bool)
        df_tmp = df_in.loc[mask].copy()
        # df_tmp = df_in.loc[df_in[COL_SOUT_ASS_FLAG]].copy()
        if df_tmp.empty:
            return pd.DataFrame(columns=[COL_SHIP_TO, COL_ITEM, COL_LOCATION,
                                        COL_SALES_PRODUCT_ASN, sout_col])    
        # 보조 플래그: Y→1, 그 외→0
...
<<<<<<< HEAD
    ...
    if df_in_estore is not None and not df_in_estore.empty and (COL_SHIP_TO in df_in_estore.columns):
        ...
        # 0-1) df_in_sout_assort 필터링
        if not df_in_sout_assort.empty and (COL_SHIP_TO in df_in_sout_assort.columns):
            df_in_sout_assort = df_in_sout_assort[
                df_in_sout_assort[COL_SHIP_TO].astype("string").isin(estore_set)
            ].copy()

        # 0-2) sout_dict 내 각 DF 필터링
        if sout_dict:
            new_sout_dict: dict[str, pd.DataFrame] = {}
            for k, df_tmp in sout_dict.items():
                if df_tmp is not None and not df_tmp.empty and (COL_SHIP_TO in df_tmp.columns):
                    new_sout_dict[k] = df_tmp[
                        df_tmp[COL_SHIP_TO].astype("string").isin(estore_set)
                    ].copy()
                else:
                    new_sout_dict[k] = df_tmp
...
def _get_current_week(df_in_Time: pd.DataFrame) -> str | None:
    ...
    첫 번째 Partial Week에서 'A'/'B' 접미사를 제거한 Week 코드(예: '202522')를 반환.
    """
    pws = _get_current_partial_weeks(df_in_Time)  # 예: ['202522A','202522B'] 또는 ['202522A']
    if not pws:
        return None
    s = str(pws[0])
...
#       sin_dummy_dict         : Step 3-1 결과 dict (S/In FCST(GI)_Dummy)  # 25.11.22 추가
#
#   - 동작:
#       • Sales Product ASN: Y→0, N→NaN (25.11.13 스펙)
#       • 각 행에 현재 주차 Partial Week(A/B 존재 시 둘 다) 부착
#       • groupby(집계) 없음
#       • (25.11.22 추가)  — GC/AP2/AP1/LOCAL **모두** 동일하게 적용
#           - 해당 measure에 대응하는 Step 3-1 결과(sin_dummy_dict)의 S/In Dummy == 0 인
#             ShipTo*Item 조합이,
#             현재 Step 3-2 결과에 S/Out Dummy 가 없으면
#             Location='-' 행을 추가 생성 (Dummy=0.0)
########################################################################################################################
@_decoration_
...
<<<<<<< HEAD
    ...
    MAP_SOUT2SIN: dict[str, tuple[str, str]] = {
        ...
        COL_SOUT_DUMMY_AP2:   (OUT_SIN_DUMMY_AP2,   COL_SIN_DUMMY_AP2),
        COL_SOUT_DUMMY_AP1:   (OUT_SIN_DUMMY_AP1,   COL_SIN_DUMMY_AP1),
        COL_SOUT_DUMMY_LOCAL: (OUT_SIN_DUMMY_LOCAL, COL_SIN_DUMMY_LOCAL),
    }
...
<<<<<<< HEAD
        ...
        if df_base is None or df_base.empty:
            out[out_name] = pd.DataFrame(
                columns=[COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK, dummy_col]
            )
            continue

        # ---------------------------------------------------------------------
        # (1) Y/N → Dummy (Y→0, N→NaN)  [기존 로직 유지]
        # ---------------------------------------------------------------------
        df_tmp = df_base[[COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_SALES_PRODUCT_ASN]].copy(deep=False)
        df_tmp[dummy_col] = _yn_to_dummy(df_tmp[COL_SALES_PRODUCT_ASN])
        df_tmp.drop(columns=[COL_SALES_PRODUCT_ASN], inplace=True)
        ...
        # ---------------------------------------------------------------------
        # (2) 25.11.22 추가 — Step 3-1(S/In Dummy) 기반 누락 S/Out 생성 (모든 measure 공통)
        #
        #   스펙 요약:
        #     - S/Out Simul Master 상관 없이 Data를 만들어야 함.
        #     - Step 3-1 의 Ship To * Item 과 Key 값을 비교하여,
        #       S/In FCST(GI)_X_Dummy == 0 일 때,
        #       Step 3-2에 없으면 Location 을 '-' 로 하여 생성
        #
        #   구현:
        #     - dummy_col (S/Out Dummy) ↔ (OUT_SIN_DUMMY_x, COL_SIN_DUMMY_x) 매핑
        #     - sin_dummy_dict[OUT_SIN_DUMMY_x] 에서 S/In Dummy_x == 0 인 ShipTo*Item 조회
        #     - df_tmp 에 동일 ShipTo*Item 이 없으면 Location='-' 로 행 추가(dummy_col=0.0)
        # ---------------------------------------------------------------------
        if sin_dummy_dict is not None and dummy_col in MAP_SOUT2SIN:
            sin_dict_key, sin_col = MAP_SOUT2SIN[dummy_col]
            df_sin = sin_dummy_dict.get(sin_dict_key, pd.DataFrame())

            if (
                df_sin is not None
                and not df_sin.empty
                and sin_col in df_sin.columns
            ):
                # (2-1) 해당 measure의 S/In Dummy == 0 인 ShipTo*Item 목록
                sin_keys = (
                    df_sin.loc[df_sin[sin_col] == 0,
                               [COL_SHIP_TO, COL_ITEM]]
                    .drop_duplicates(ignore_index=True)
                )

                if not sin_keys.empty:
                    # (2-2) 현재 S/Out Dummy 결과에 존재하는 ShipTo*Item 목록
                    if df_tmp is None or df_tmp.empty:
                        sout_keys = pd.DataFrame(columns=[COL_SHIP_TO, COL_ITEM])
                    else:
                        sout_keys = df_tmp[[COL_SHIP_TO, COL_ITEM]].drop_duplicates(ignore_index=True)

                    # (2-3) anti-join: sin_keys - sout_keys  → 누락된 S/Out 대상
                    if sout_keys.empty:
                        missing = sin_keys
                    else:
                        missing = sin_keys.merge(
                            sout_keys.assign(__hit__=1),
                            on=[COL_SHIP_TO, COL_ITEM],
                            how='left'
                        )
                        missing = missing[missing['__hit__'].isna()].drop(columns=['__hit__'])

                    # (2-4) 누락된 ShipTo*Item 에 대해 Location='-' + dummy_col=0.0 행 생성
                    if not missing.empty:
                        missing = missing.copy()
                        missing[COL_LOCATION] = '-'   # 스펙: Location 을 '-' 로 하여 생성
                        missing[dummy_col]    = 0.0   # S/Out Dummy 값도 0 (선택 상태)
                        # df_tmp와 동일한 컬럼 순서로 맞춰서 concat
                        missing = missing[[COL_SHIP_TO, COL_ITEM, COL_LOCATION, dummy_col]]
                        df_tmp = pd.concat([df_tmp, missing], ignore_index=True)

        # ---------------------------------------------------------------------
        # (3) Partial Week 확장 (ShipTo*Item*Location × current Partial Weeks)
        #     - groupby/집계 없이 단순 카티전 곱
        # ---------------------------------------------------------------------
        df_out = _expand_with_partial_weeks(df_tmp, partial_weeks, dummy_col)
...
<<<<<<< HEAD
    key_all   = (df_asn_base[COL_SHIP_TO].astype("string") + '|' +
                 ...
                 df_asn_base[COL_SALES_PRODUCT_ASN].astype("string")).unique()
    dup_set = set(key_all)

    key_delta = (df_delta[COL_SHIP_TO].astype("string") + '|' +
...
<<<<<<< HEAD
    ...
    • 값 변환: Y→1, N→NaN
    • 당주 한 주(Week)만 1건으로 생성 (A/B 확장 삭제)
    """
    # 0) 당주 Week 계산
    cur_week = _get_current_week(df_in_Time)
    if df_flooring_pop is None or df_flooring_pop.empty or not cur_week:
        # 스키마만 맞춘 빈 DF
        df_empty = pd.DataFrame(columns=[COL_SHIP_TO, COL_ITEM, COL_WEEK, COL_FLOORING_DUMMY])
        for c in (COL_SHIP_TO, COL_ITEM, COL_WEEK):
            df_empty[c] = df_empty[c].astype('category')
        df_empty[COL_FLOORING_DUMMY] = df_empty[COL_FLOORING_DUMMY].astype('float32')
        return df_empty    
    # 1) 필요한 컬럼만 취하고 값 변환(Y/N → 1/NaN)
    df = df_flooring_pop[[COL_SHIP_TO, COL_ITEM, COL_SALES_PRODUCT_ASN]].copy(deep=False)
    df[COL_FLOORING_DUMMY] = _yn_to_dummy(df[COL_SALES_PRODUCT_ASN])
    df.drop(columns=[COL_SALES_PRODUCT_ASN], inplace=True)

    # 2) 당주 Week 부여 (A/B 확장 없음)
    df[COL_WEEK] = cur_week

    # 3) 컬럼 순서/타입 정리
    df = df[[COL_SHIP_TO, COL_ITEM, COL_WEEK, COL_FLOORING_DUMMY]].drop_duplicates(ignore_index=True)
    for c in (COL_SHIP_TO, COL_ITEM, COL_WEEK):
        if df[c].dtype.name != 'category':
            df[c] = df[c].astype('category')
...
<<<<<<< HEAD
    def _fmt(df_in: pd.DataFrame, meas_col: str, with_loc: bool = True, *, time_col: str = COL_PWEEK) -> pd.DataFrame:
        ...
        if df_in is None or df_in.empty:
            ...
            # dtype 정리
            for c in [COL_VERSION, COL_SHIP_TO, COL_ITEM] + ([COL_LOCATION] if with_loc else []) + [time_col]:
                df[c] = df[c].astype('category')
        ...
        if with_loc:
            if COL_LOCATION not in df.columns:
                df[COL_LOCATION] = '-'
            if df[COL_LOCATION].dtype.name != 'category':
                df[COL_LOCATION] = df[COL_LOCATION].astype('category')
        else:
            # Location이 있어도 출력 스키마에는 포함하지 않음
            if COL_LOCATION in df.columns:
                df = df.drop(columns=[COL_LOCATION])

        # 시간 컬럼 처리 (Partial/Week 중 호출자가 지정)
        if time_col not in df.columns:
            # 혹시 반대 축이 들어왔으면 안전하게 rename (방어코드)
            alt = COL_WEEK if time_col == COL_PWEEK else COL_PWEEK
            if alt in df.columns:
                df = df.rename(columns={alt: time_col})
            else:
                # 없으면 빈 category로 생성
                df[time_col] = pd.Series(dtype='category')
        if df[time_col].dtype.name != 'category':
            df[time_col] = df[time_col].astype('category')
        ...
        if df[COL_VERSION].dtype.name != 'category':
...
<<<<<<< HEAD
    ...
    out[OUT_SIN_DUMMY_AP1]   = _fmt(sin_dummy_dict.get(OUT_SIN_DUMMY_AP1),   COL_SIN_DUMMY_AP1,   with_loc=True,  time_col=COL_PWEEK)
    out[OUT_SIN_DUMMY_LOCAL] = _fmt(sin_dummy_dict.get(OUT_SIN_DUMMY_LOCAL), COL_SIN_DUMMY_LOCAL, with_loc=True,  time_col=COL_PWEEK)

    # S/Out (Partial Week)
    out[OUT_SOUT_DUMMY_GC]    = _fmt(sout_dummy_dict.get(OUT_SOUT_DUMMY_GC),    COL_SOUT_DUMMY_GC,    with_loc=True,  time_col=COL_PWEEK)
    out[OUT_SOUT_DUMMY_AP2]   = _fmt(sout_dummy_dict.get(OUT_SOUT_DUMMY_AP2),   COL_SOUT_DUMMY_AP2,   with_loc=True,  time_col=COL_PWEEK)
    out[OUT_SOUT_DUMMY_AP1]   = _fmt(sout_dummy_dict.get(OUT_SOUT_DUMMY_AP1),   COL_SOUT_DUMMY_AP1,   with_loc=True,  time_col=COL_PWEEK)
    out[OUT_SOUT_DUMMY_LOCAL] = _fmt(sout_dummy_dict.get(OUT_SOUT_DUMMY_LOCAL), COL_SOUT_DUMMY_LOCAL, with_loc=True,  time_col=COL_PWEEK)

    # Flooring (Week) — Location 없음
...
<<<<<<< HEAD
    ...
    sout_dummy_dict = fn_step03_02_build_sout_dummy_current_pw(
        df_step02_04_sout_dict,
        df_in_Time,
        sin_dummy_dict    # ★ 25.11.22 추가
    )
...
#   • df_in_Time_all 의 Partial Week(당주 ~ +52주) 전체로 확장
#   • Measure 이름을 Split Ratio_* 로 변경, 값은 NaN 유지(= 삭제/초기화 의미)
#   • 결과가 없으면 스키마만 맞는 빈 DF 반환
# ─────────────────────────────────────────────────────────────────────────────

# ─────────────────────────────────────────────
# 컬럼 상수 (Columns Step 4)
# ─────────────────────────────────────────────
# Split Ratio (S/In)
...
# ======================================================
# 데이터프레임 상수 (Input Step 4)
# ======================================================
DF_IN_TIME_ALL = 'df_in_Time_all'   # (25.10.10 추가) 당주~+52주 Partial Week

# ======================================================
# 데이터프레임 상수 (Output - Step 4)
# ======================================================
OUT_SIN_SPLIT_GC       = 'df_output_Sell_In_FCST_GI_Split_Ratio_GC'
OUT_SIN_SPLIT_AP2      = 'df_output_Sell_In_FCST_GI_Split_Ratio_AP2'
OUT_SIN_SPLIT_AP1      = 'df_output_Sell_In_FCST_GI_Split_Ratio_AP1'
OUT_SIN_SPLIT_LOCAL    = 'df_output_Sell_In_FCST_GI_Split_Ratio_Local'

OUT_SOUT_SPLIT_GC      = 'df_output_Sell_Out_FCST_Split_Ratio_GC'
OUT_SOUT_SPLIT_AP2     = 'df_output_Sell_Out_FCST_Split_Ratio_AP2'
OUT_SOUT_SPLIT_AP1     = 'df_output_Sell_Out_FCST_Split_Ratio_AP1'
OUT_SOUT_SPLIT_LOCAL   = 'df_output_Sell_Out_FCST_Split_Ratio_Local'

@_decoration_
def fn_step04_build_split_ratio_outputs(
    df_step02_05_out: dict[str, pd.DataFrame],  # Step 2-5 결과 dict (8개)
    df_in_Time_all: pd.DataFrame,               # Input 12 (당주~+52주 Partial Week 전체)
    out_version: str
) -> dict[str, pd.DataFrame]:    # 0) 방어: Time all
    if df_in_Time_all is None or df_in_Time_all.empty or (COL_PWEEK not in df_in_Time_all.columns):
        # 모든 출력에 대해 빈 DF 반환
        def _empty(cols):
            df = pd.DataFrame(columns=cols)
            for c in [COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK]:
                if c in df.columns:
                    df[c] = df[c].astype('category')
            if cols[-1] not in (COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK):
                df[cols[-1]] = df[cols[-1]].astype('float32')
            return df

        return {
            OUT_SIN_SPLIT_GC      : _empty([COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK, COL_SIN_SPLIT_GC]),
            OUT_SIN_SPLIT_AP2     : _empty([COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK, COL_SIN_SPLIT_AP2]),
            OUT_SIN_SPLIT_AP1     : _empty([COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK, COL_SIN_SPLIT_AP1]),
            OUT_SIN_SPLIT_LOCAL   : _empty([COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK, COL_SIN_SPLIT_LOCAL]),
            OUT_SOUT_SPLIT_GC     : _empty([COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK, COL_SOUT_SPLIT_GC]),
            OUT_SOUT_SPLIT_AP2    : _empty([COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK, COL_SOUT_SPLIT_AP2]),
            OUT_SOUT_SPLIT_AP1    : _empty([COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK, COL_SOUT_SPLIT_AP1]),
            OUT_SOUT_SPLIT_LOCAL  : _empty([COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK, COL_SOUT_SPLIT_LOCAL]),
        }

    # 1) Partial Week 목록 (당주~+52주)
    pweeks_all = df_in_Time_all[COL_PWEEK].astype("string").tolist()

    # 2) 매핑 (Step 2-5 → Step 4)
    #   left : Step 2-5 dict 키 (Assortment 출력)
    #   mid1 : Step 2-5 측정치 컬럼명
    #   mid2 : Step 4 Split Ratio 측정치 컬럼명
    #   right: Step 4 dict 키 (출력 DF 이름)
    MAP_SIN = [
        (OUT_SIN_ASSORTMENT_GC,    COL_SIN_ASSORT_GC,    COL_SIN_SPLIT_GC,    OUT_SIN_SPLIT_GC),
        (OUT_SIN_ASSORTMENT_AP2,   COL_SIN_ASSORT_AP2,   COL_SIN_SPLIT_AP2,   OUT_SIN_SPLIT_AP2),
        (OUT_SIN_ASSORTMENT_AP1,   COL_SIN_ASSORT_AP1,   COL_SIN_SPLIT_AP1,   OUT_SIN_SPLIT_AP1),
        (OUT_SIN_ASSORTMENT_LOCAL, COL_SIN_ASSORT_LOCAL, COL_SIN_SPLIT_LOCAL, OUT_SIN_SPLIT_LOCAL),
    ]
    MAP_SOUT = [
        (OUT_SOUT_ASSORTMENT_GC,    COL_SOUT_ASSORT_GC,    COL_SOUT_SPLIT_GC,    OUT_SOUT_SPLIT_GC),
        (OUT_SOUT_ASSORTMENT_AP2,   COL_SOUT_ASSORT_AP2,   COL_SOUT_SPLIT_AP2,   OUT_SOUT_SPLIT_AP2),
        (OUT_SOUT_ASSORTMENT_AP1,   COL_SOUT_ASSORT_AP1,   COL_SOUT_SPLIT_AP1,   OUT_SOUT_SPLIT_AP1),
        (OUT_SOUT_ASSORTMENT_LOCAL, COL_SOUT_ASSORT_LOCAL, COL_SOUT_SPLIT_LOCAL, OUT_SOUT_SPLIT_LOCAL),
    ]

    def _build_one(df_assort: pd.DataFrame, meas_in: str, meas_out: str) -> pd.DataFrame:
        """
        • Step 2-5 결과에서 meas_in 이 NaN(= 남겨진 행)인 키만 추출
        • 키(ShipTo, Item, Loc) × TimeAll 로 확장
        • meas_out 컬럼은 NaN 유지(= 삭제/초기화 의미)
        """
        cols = [COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK, meas_out]

        if df_assort is None or df_assort.empty or (meas_in not in df_assort.columns):
            df = pd.DataFrame(columns=cols)
            for c in (COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK):
                df[c] = df[c].astype('category')
            df[meas_out] = df[meas_out].astype('float32')
            return df

        # 2-5 스펙(25.10.10) 반영: Assortment가 Null 인 행만 사용
        df_assort = df_assort[df_assort[meas_in].isna()]
        ...
        # 키만 추출(중복 제거)
        base = (df_assort[[COL_SHIP_TO, COL_ITEM, COL_LOCATION]]
                .drop_duplicates(ignore_index=True))

        # Split Ratio 컬럼 준비(전부 NaN)
        base[meas_out] = np.nan

        # Partial Week 확장 (기존 벡터화 헬퍼 재사용)
        df_expanded = _expand_with_partial_weeks(base, pweeks_all, meas_out)

        # Version 및 dtypes/순서 정리
        df_expanded.insert(0, COL_VERSION, out_version)
        df_expanded[COL_VERSION] = df_expanded[COL_VERSION].astype('category')
        df_expanded[meas_out] = pd.to_numeric(df_expanded[meas_out], errors='coerce').astype('float32')

        return df_expanded[[COL_VERSION, COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_PWEEK, meas_out]]

    out_dict: dict[str, pd.DataFrame] = {}

    # S/In 4종
    for in_key, meas_in, meas_out, out_key in MAP_SIN:
        df_src = df_step02_05_out.get(in_key, pd.DataFrame())
        out_dict[out_key] = _build_one(df_src, meas_in, meas_out)

    # S/Out 4종
    for in_key, meas_in, meas_out, out_key in MAP_SOUT:
        df_src = df_step02_05_out.get(in_key, pd.DataFrame())
        out_dict[out_key] = _build_one(df_src, meas_in, meas_out)

    # 메모리 정리
    del df_step02_05_out, df_in_Time_all
    gc.collect()

    return out_dict

# ======================================================
# Step 5-1) Assortment 값 기반으로 Sales Product ASN 생성
#
# 목적
#   - Step 2-2 / Step 2-3 산출물(sin_dict, df_step02_03_sin_dict)에서
#     "Assortment 컬럼은 제거"하고, 남아있는 Sales Product ASN(Y/N)을 취합하여
#     (ShipTo, Item, Location) 단위의 Sales Product ASN을 생성한다.
#
# 입력
#   - sin_dict               : Step 2-2 결과 dict (4개 DF: GC/AP2/AP1/Local)
#   - df_step02_03_sin_dict  : Step 2-3 결과 dict (4개 DF: GC/AP2/AP1/Local)
#
# 전제(스펙)
#   - Step 2-2/2-3 결과 DF에는 기본적으로 아래가 포함됨
#       [ShipTo, Item, Location, Sales Product ASN, (Assortment_xxx ...)]
#   - Assortment_xxx 컬럼은 Step 5-1에서 모두 삭제/무시한다.
#   - Sales Product ASN 값 규칙
#       'Y' -> 'Y' 유지
#       'N' -> Null 처리 (pd.NA)
#       그 외/공백/NaN -> Null 처리
#
# 처리(재정리된 스펙 그대로)
#   (1) sin_dict + df_step02_03_sin_dict 의 모든 DF들을 대상으로
#       - [ShipTo, Item, Location, Sales Product ASN] 만 추출
#       - ASN 값 정규화: 'Y'만 남기고 나머지 Null
#   (2) 전부 concat하여 하나로 합침
#   (3) (ShipTo, Item, Location) 단위로 집계:
#       - 어느 소스(어느 measure)에서든 ASN='Y'가 하나라도 있으면 최종 'Y'
#       - 하나도 없으면 최종 Null
#   (4) Sales Product ASN Leaf Flag 컬럼 신규 추가:
#       - 값은 전부 pd.NA
#       - dtype: string으로 만든 뒤 category로 변환 (스펙 요구)
#
# 출력
#   - df_step05_01
#     [ShipTo, Item, Location, Sales Product ASN, Sales Product ASN Leaf Flag]
# ======================================================
@_decoration_
def fn_step05_01_build_sales_product_asn_from_assortments(
    sin_dict: dict[str, pd.DataFrame],                 # Step 2-2 결과 (4개)
    df_step02_03_sin_dict: dict[str, pd.DataFrame],    # Step 2-3 결과 (4개)
    **kwargs
) -> pd.DataFrame:

    # ─────────────────────────────────────────────────────
    # 0) 내부 유틸: DF 1개에서 필요한 컬럼만 뽑고 ASN 정규화
    # ─────────────────────────────────────────────────────
    def _pick_rows(df: pd.DataFrame) -> pd.DataFrame:
        """
        • 입력 DF가 비었으면 빈 스키마 반환
        • 필수컬럼(ShipTo,Item,Location,Sales Product ASN)이 없으면 빈 스키마 반환
          - dict마다 스키마 차이가 있을 수 있으므로 Step5-1에서는 에러 대신 skip 성격
        • ASN 정규화: 'Y'만 유지, 나머지 Null(pd.NA)
        """
        need = [COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_SALES_PRODUCT_ASN]
        ...
        w = df[need].copy(deep=False)

        # ASN 정규화: 'Y'만 남기고 나머지는 Null
        s = w[COL_SALES_PRODUCT_ASN].astype('string').str.strip().str.upper()
        w[COL_SALES_PRODUCT_ASN] = np.where(s.eq('Y'), 'Y', pd.NA)

        return w

    # ─────────────────────────────────────────────────────
    # 1) Step 2-2 / Step 2-3 dict의 DF들을 모두 수집(concat용)
    #    - 여기서 "Assortment 컬럼 삭제"는 사실상 "필요 컬럼만 추출"로 구현됨
    # ─────────────────────────────────────────────────────
    frames: list[pd.DataFrame] = []

    for df in (sin_dict or {}).values():
        frames.append(_pick_rows(df))

    for df in (df_step02_03_sin_dict or {}).values():
        frames.append(_pick_rows(df))

    # dict 자체가 비어있거나 전부 빈 DF이면 빈 결과 반환
    if not frames:
        return pd.DataFrame(columns=[
            COL_SHIP_TO, COL_ITEM, COL_LOCATION,
            COL_SALES_PRODUCT_ASN, COL_SALES_PRODUCT_ASN_LEAF
        ])
    ...
    # ─────────────────────────────────────────────────────
    # 2) (ShipTo, Item, Location) 단위로 최종 ASN 결정
    #    - 어느 DF에서든 Y 하나라도 있으면 최종 Y
    #    - 아니면 최종 Null
    # ─────────────────────────────────────────────────────
    # Y 여부를 숫자 플래그로 변환해 집계(최대값)
    work['__y__'] = (
        work[COL_SALES_PRODUCT_ASN]
    ...
    # 최종 ASN 재구성
    work[COL_SALES_PRODUCT_ASN] = np.where(
        work['__y__'].fillna(0).astype(float) > 0,
        'Y',
        pd.NA
    )
    work.drop(columns=['__y__'], inplace=True)

    # ─────────────────────────────────────────────────────
    # 3) Leaf Flag 컬럼 추가 (스펙: 값은 np.NA / string → category)
    # ─────────────────────────────────────────────────────
    work[COL_SALES_PRODUCT_ASN_LEAF] = pd.Series([pd.NA] * len(work), dtype="string")
    work[COL_SALES_PRODUCT_ASN_LEAF] = work[COL_SALES_PRODUCT_ASN_LEAF].astype('category')

    # ─────────────────────────────────────────────────────
    # 4) dtype 정리 (메모리 절감 & 기존 코드 스타일 유지)
    # ─────────────────────────────────────────────────────
    for c in (COL_SHIP_TO, COL_ITEM, COL_LOCATION):
        if c in work.columns:
            work[c] = work[c].astype('category')

    work[COL_SALES_PRODUCT_ASN] = work[COL_SALES_PRODUCT_ASN].astype('string').astype('category')

    return work[[COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_SALES_PRODUCT_ASN, COL_SALES_PRODUCT_ASN_LEAF]]


# ======================================================
# Step 5-2) Step 1-4 결과 + Step 5-1 결과 결합하여 최종 Output_Sales_Product_ASN 생성
#
# 목적
#   - Step 5 최종 산출물은 Output_Sales_Product_ASN이며,
#     이는 Step 1-4(Output_Sales_Product_ASN: Leaf Flag만 존재)를 "덮어쓴다".
#
# 입력
#   - df_step05_01 : Step 5-1 결과
#       [ShipTo, Item, Location, Sales Product ASN, Sales Product ASN Leaf Flag(=NA)]
#   - df_step01_04 : Step 1-4 결과 (이미 생성되어 있음)
#       [Version, ShipTo, Item, Location, Sales Product ASN Leaf Flag]
#   - out_version  : 최종 Version 값
#
# 처리(스펙)
#   (1) Step 1-4에서 Leaf Flag를 복사하여 'Sales Product ASN' 컬럼을 만든다.
#       - Leaf Flag는 'Y'만 인정, 나머지는 Null
#       - 복사 결과 ASN 후보도 'Y'만 인정, 나머지 Null
#   (2) Step 5-1과 Step 1-4를 (ShipTo, Item, Location) 기준 outer merge
#       - 키 통합(둘 중 한쪽에만 있어도 결과에 포함)
#   (3) 최종 컬럼 결정
#       - Sales Product ASN:
#           Step 5-1 ASN 우선, 없으면 Step 1-4에서 복사된 ASN 사용, 그 외 Null
#       - Sales Product ASN Leaf Flag:
#           Step 1-4의 Leaf Flag를 최종값으로 사용(없으면 Null)
#   (4) Version 주입 및 dtype category 정리
#
# 출력
#   - Output_Sales_Product_ASN
#     [Version, ShipTo, Item, Location, Sales Product ASN, Sales Product ASN Leaf Flag]
# ======================================================
@_decoration_
def fn_step05_02_build_output_sales_product_asn(
    df_step05_01: pd.DataFrame,      # Step 5-1 결과
    df_step01_04: pd.DataFrame,      # Step 1-4 결과 (Leaf Flag only)
    out_version: str,
    **kwargs
) -> pd.DataFrame:
    ...
    # 0) 방어: None 처리
    # ─────────────────────────────────────────────────────
    df_step05_01 = df_step05_01 if df_step05_01 is not None else pd.DataFrame()
    df_step01_04 = df_step01_04 if df_step01_04 is not None else pd.DataFrame()

    # ─────────────────────────────────────────────────────
    # 1) Step 1-4 결과를 Step 5-2에서 사용하기 위한 형태로 변환
    #    - Leaf Flag를 정규화('Y'만 인정)
    #    - Leaf Flag를 복사하여 ASN 후보 컬럼 생성
    # ─────────────────────────────────────────────────────
    if not df_step01_04.empty:
        need_14 = [COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_SALES_PRODUCT_ASN_LEAF]
        miss = [c for c in need_14 if c not in df_step01_04.columns]
        if miss:
            raise KeyError(f"[Step 5-2] Step1-4 DF missing columns: {miss}")

        df14 = df_step01_04[need_14].copy(deep=False)

        # Leaf Flag 정규화: 'Y'만 유지
        s_leaf = df14[COL_SALES_PRODUCT_ASN_LEAF].astype('string').str.strip().str.upper()
        df14[COL_SALES_PRODUCT_ASN_LEAF] = np.where(s_leaf.eq('Y'), 'Y', pd.NA)

        # Leaf Flag -> ASN 후보 생성 (스펙: Leaf Flag 복사해 Sales Product ASN 만든다)
        df14[COL_SALES_PRODUCT_ASN] = df14[COL_SALES_PRODUCT_ASN_LEAF]

        # Step 5-1과 merge할 스키마로 정렬
        df14 = df14[[COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_SALES_PRODUCT_ASN, COL_SALES_PRODUCT_ASN_LEAF]]
    else:
        df14 = pd.DataFrame(columns=[
            COL_SHIP_TO, COL_ITEM, COL_LOCATION,
            COL_SALES_PRODUCT_ASN, COL_SALES_PRODUCT_ASN_LEAF
        ])

    # ─────────────────────────────────────────────────────
    # 2) Step 5-1 결과 스키마 확인/정렬
    # ─────────────────────────────────────────────────────
    if not df_step05_01.empty:
        need_51 = [COL_SHIP_TO, COL_ITEM, COL_LOCATION, COL_SALES_PRODUCT_ASN, COL_SALES_PRODUCT_ASN_LEAF]
        miss = [c for c in need_51 if c not in df_step05_01.columns]
        if miss:
            raise KeyError(f"[Step 5-2] Step5-1 DF missing columns: {miss}")

        df51 = df_step05_01[need_51].copy(deep=False)
    else:
        df51 = pd.DataFrame(columns=[
            COL_SHIP_TO, COL_ITEM, COL_LOCATION,
            COL_SALES_PRODUCT_ASN, COL_SALES_PRODUCT_ASN_LEAF
        ])
    ...
    # ─────────────────────────────────────────────────────
    # 3) outer merge로 키 통합
    #    - Step5-1 우선 적용을 위해 suffix로 구분
    # ─────────────────────────────────────────────────────
    merged = df51.merge(
        df14,
        on=[COL_SHIP_TO, COL_ITEM, COL_LOCATION],
        how='outer',
        suffixes=('_51', '_14')
    )
    ...
    # ─────────────────────────────────────────────────────
    # 4) 최종 값 결정
    #    - ASN: Step5-1(우선) -> Step1-4복사ASN(차선)
    #    - LeafFlag: Step1-4의 LeafFlag 최우선 (없으면 NA)
    # ─────────────────────────────────────────────────────
    def _norm_y(s: pd.Series) -> pd.Series:
        if s is None:
            return pd.Series([pd.NA] * len(merged), index=merged.index)
        t = s.astype('string').astype(str).str.strip().str.upper()
        return pd.Series(np.where(t.eq('Y'), 'Y', pd.NA), index=merged.index)

    asn_51   = _norm_y(merged.get(f"{COL_SALES_PRODUCT_ASN}_51"))
    asn_14   = _norm_y(merged.get(f"{COL_SALES_PRODUCT_ASN}_14"))
    leaf_14  = _norm_y(merged.get(f"{COL_SALES_PRODUCT_ASN_LEAF}_14"))

    final_asn  = asn_51.fillna(asn_14)      # Step5-1 우선
    final_leaf = leaf_14                   # Step1-4 Leaf Flag가 최종

    # ─────────────────────────────────────────────────────
    # 5) 최종 Output 구성 + Version 주입
    # ─────────────────────────────────────────────────────
    out = pd.DataFrame({
        COL_VERSION: out_version,
        COL_SHIP_TO: merged[COL_SHIP_TO],
        COL_ITEM: merged[COL_ITEM],
        COL_LOCATION: merged[COL_LOCATION],
        COL_SALES_PRODUCT_ASN: final_asn,
        COL_SALES_PRODUCT_ASN_LEAF: final_leaf
    })
...
<<<<<<< HEAD
        ...
        dict_log = {
            'p_step_no'  : 41,
            'p_step_desc': 'Step 4) Split Ratio Output 구성',
            'p_df_name'  : None
        }
        df_step04_split_ratio_dict = fn_step04_build_split_ratio_outputs(
            df_step02_05_out,                    # Step 2-5 결과 dict (8개)
            input_dataframes[DF_IN_TIME_ALL],    # Input 12: 당주 ~ +52 Partial Week
            Version,
            **dict_log
        )
        # 8개 Output 개별 변수로 전개 (o9와 인터페이스 맞춤)
        df_output_Sell_In_FCST_GI_Split_Ratio_GC     = df_step04_split_ratio_dict[OUT_SIN_SPLIT_GC]
        df_output_Sell_In_FCST_GI_Split_Ratio_AP2    = df_step04_split_ratio_dict[OUT_SIN_SPLIT_AP2]
        df_output_Sell_In_FCST_GI_Split_Ratio_AP1    = df_step04_split_ratio_dict[OUT_SIN_SPLIT_AP1]
        df_output_Sell_In_FCST_GI_Split_Ratio_Local  = df_step04_split_ratio_dict[OUT_SIN_SPLIT_LOCAL]
        df_output_Sell_Out_FCST_Split_Ratio_GC       = df_step04_split_ratio_dict[OUT_SOUT_SPLIT_GC]
        df_output_Sell_Out_FCST_Split_Ratio_AP2      = df_step04_split_ratio_dict[OUT_SOUT_SPLIT_AP2]
        df_output_Sell_Out_FCST_Split_Ratio_AP1      = df_step04_split_ratio_dict[OUT_SOUT_SPLIT_AP1]
        df_output_Sell_Out_FCST_Split_Ratio_Local    = df_step04_split_ratio_dict[OUT_SOUT_SPLIT_LOCAL]

        # 로깅
        for name, df_out in df_step04_split_ratio_dict.items():
            fn_log_dataframe(df_out, f'step04_{name}')


        ################################################################################################################
        # Step 5-1) Assortment 기반 Sales Product ASN 생성
        #   - Step 2-2(sin_dict), Step 2-3(df_step02_03_sin_dict) 결과를 합쳐
        #     (ShipTo, Item, Location) 단위의 Sales Product ASN을 생성한다.
        ################################################################################################################
        dict_log = {
            'p_step_no'  : 51,
            'p_step_desc': 'Step 5-1) Assortment 기반 Sales Product ASN 생성',
            'p_df_name'  : None
        }
        df_step05_01 = fn_step05_01_build_sales_product_asn_from_assortments(
            sin_dict,               # Step 2-2 결과 dict (GC/AP2/AP1/Local)
            df_step02_03_sin_dict,   # Step 2-3 결과 dict (GC/AP2/AP1/Local)
            **dict_log
        )
        fn_log_dataframe(df_step05_01, 'df_step05_01')

        ################################################################################################################
        # Step 5-2) Step 1-4 + Step 5-1 결합 → 최종 Output_Sales_Product_ASN 생성
        #   - Step 5 결과가 최종 Output이며, Step 1-4 결과를 덮어쓴다.
        ################################################################################################################
        dict_log = {
            'p_step_no'  : 52,
            'p_step_desc': 'Step 5-2) Output_Sales_Product_ASN 생성(덮어쓰기)',
            'p_df_name'  : None
        }
        df_step05_02_out_asn = fn_step05_02_build_output_sales_product_asn(
            df_step05_01,             # Step 5-1 결과
            Output_Sales_Product_ASN, # Step 1-4 결과(LeafFlag only)
            Version,
            **dict_log
        )
        fn_log_dataframe(df_step05_02_out_asn, 'df_step05_02_out_asn')

        # 최종 Output은 Step5 산출로 덮어쓰기
